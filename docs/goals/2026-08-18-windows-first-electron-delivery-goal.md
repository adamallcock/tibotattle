...
The dated Windows qualification notes below are historical checkpoints, not
evidence for the current parity integration. The current source boundary starts
at `origin/main` revision `8687cbb9ce0cdad63893b726381e40a73099f09b` with package
version `0.1.16`, and its Electron changes still require a fresh native Windows
x64 warm-and-clean run before they can be accepted. The earlier `0.1.15`
revision and run identifiers remain provenance records only; they must not be
reused for the current source, package bytes, or production-signing claim.

The native macOS application remains unchanged. Until a fresh current-revision
qualification receipt proves otherwise, `productionSafe` and all production
selectors remain false, and no Windows or Linux support claim is open.

The signed finalizer workflow is now present in this integration as an
unpublished, activation-gated two-job design. It has not run, and no Azure
session, native signing, or signed artifact has been produced. The historical
statement below that the protected finalizer did not exist is superseded by
...
## Superseded historical evidence — 2026-08-21 — content-free provenance join
...
- At that historical checkpoint, the protected production finalizer workflow
  did not exist or run. This statement is superseded by the current
  2026-08-23 checkpoint above. No Azure or native Windows proof, signing,
  selector enablement, version bump, release, push, or Windows-support claim
  occurred at that historical checkpoint; `productionSafe` and all production
  selectors remained false.

## Superseded historical evidence — latest bounded outcome: 2026-08-21 (exact run `32477262214`)
