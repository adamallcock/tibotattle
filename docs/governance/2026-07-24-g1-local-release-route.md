...
| Failed-workspace discard | Separate export/application/platform owners and exact-binding facades pass the current focused crash, hostile-input, pre-write validation, and recovery checks within the declared same-user threat boundary | Exact-inventory tests, descriptor/adversarial owner matrix, real post-commit SIGKILL, and explicit receipt/journal/marker filesystem-failure states followed by local transient-lock cleanup and recovery | Closed only for the focused source checkpoint; full-suite, packaged uninstall, clean-host, signing, and formal release review remain open |
