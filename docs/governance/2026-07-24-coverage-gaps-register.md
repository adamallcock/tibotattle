...
| Standard versus Fast | **Partial** | Prospective and some historical thread settings distinguish subscription Standard from Fast, and Fast is priced at the published Priority (Fast) API rate for the exact registry model, event time, and input context (2026-08-30), with a disclosed assumed 2x where no matching Priority card exists. | Per-request confirmation for turns before a session's first recorded change, and empirical confirmation that the subscription meter drains at the Priority price ratio (the superseded credit-rate statement claimed 2.5x for GPT-5.6 versus the published 2x price ratio). | Wrong speed-price sensitivity and false time trend. | Reconcile a declared Fast window (for example 2026-08-05 18:25–23:21Z) against observed quota drawdown; the residual-inference bands separate the 2x and 2.5x hypotheses. |
...
- **2026-07-24:** A bounded ordinary-Chat negative control found no material
  provider movement beyond the local Codex forecast. This dated observation is
  retained in Git history; current provider-coupling claims still require live
  source verification.
- **2026-07-24:** Added `calibrate-weekly` and the owner-only seven-day calibration report at `.usage-monitor/legacy-reports/2026-07-24-weekly-7-day-calibration-report.html`. Fourteen stable resets now have exact API-price-equivalent values, chronological holdout receipts, and no-look-ahead prior-reset errors; conservative captured-speed weighting narrowly beats unweighted Standard API cost.
- **2026-07-24:** Added the `quality` command and owner-only monitoring quality report at `.usage-monitor/legacy-reports/2026-07-24-monitoring-quality-report.html`. The first profile separated fixed-reset timestamp jitter from the moving/high-churn `codex_bengalfox` family, quantified whole-percentage censoring, and exposed collector freshness as an operational gate.
