...
| --- | --- | --- | --- |
| macOS direct | Developer ID-signed, notarized, stapled DMG; arm64 and x64 as separate subjects | cleanInstallSmokePassed, developerIdSigned, hardenedRuntime, notarizationAccepted, ticketStapled, gatekeeperAssessmentPassed | Released 0.1.21 on both architectures; native 0.1.18 and Electron 0.1.20 production updates passed; provenance remains manifest-specific |
...
| Windows direct | One selected native x64 installer/package, after signing nested executables and final package | cleanInstallSmokePassed, authenticodeSigned, timestamped | Released Windows x64 NSIS installer; signed 0.1.21 installed processing, restart and uninstall passed; interactive acceptance limits remain separate |
...
| Linux direct | Native x86_64 AppImage or a package from a real signed repository | cleanInstallSmokePassed, artifactIntegrityVerified, plus distribution-specific detached/repository evidence where applicable | Released Linux x86_64 AppImage; 0.1.21 packaging passed with retained owner acceptance; this is not a new physical desktop/update test |
