...
Implement the first independently testable accounting slice of the [end-to-end multi-user usage-monitor goal](../goals/2026-07-24-end-to-end-multi-user-usage-monitor-goal.md): a local-only, provider-neutral API-price-equivalent ledger for Codex/OpenAI and Claude/Anthropic evidence. The ledger must improve the cost-versus-quota calibration without implying that public API prices are the provider's subscription allowance formula.
