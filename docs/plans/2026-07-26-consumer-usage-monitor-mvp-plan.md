...
# July 27 corrective accounting and consumer-UX pass

## Verified defect

The local companion's recent backfill projection is not currently an
authoritative accounting source. In a fixed July 27 comparison interval, the
append-only collector ledger exposed 44,343 token-count rows and 6.14 billion
tokens, while the lineage-aware scanner exposed 5,682 usage increments and
779 million tokens. The lineage-aware scan explicitly removed 39,030 inherited
fork-replay snapshots. The resulting collector inflation explains both the
implausible 91.5% subagent share and the large discrepancy between the
dashboard's $15.8k recent total and the earlier replay-safe calibration.

Until a replay-safe recent projection is available, the UI must not label the
collector total "exact" or use it as the primary cost-versus-quota comparator.

## Corrective acceptance criteria

1. Produce the dashboard's recent cost, timeline, model, speed, surface, and
   lineage accounting from the existing lineage-aware scanner or a cache made
   by that scanner; inherited fork replay must be excluded.
2. Keep raw logs local and cache only a closed, content-free projection.
3. Define one usage record as a token-count usage increment, not a user
   message, task, or tool call.
4. Price component bars in dollars in the overview; keep token quantities only
   in the accounting drill-down.
5. Treat separated text and reasoning output as mutually exclusive with a
   provider-only combined-output field so totals cannot double count both.
6. Use the browser's resolved local timezone for the primary chart and table
   labels, with UTC available as secondary evidence.
7. Pair API-price-equivalent activity with observed allowance percentage on a
   clearly labeled secondary axis where the evidence can be aligned.
8. Round consumer-facing dollars to cents or whole dollars, percentages to at
   most one decimal, and counts with compact notation. Preserve higher
   precision only in downloadable machine-readable evidence.
9. Make Overview a concise answer and Accounting its non-duplicative
   drill-down. Move unavailable API-tier and reasoning-effort dimensions, plus
   archived technical reports, behind progressive disclosure.
10. Explain every retained dimension, rename unknowns to the reason they are
    unknown, and show unpriced unknown-model usage as unpriced rather than
    `$0.00`.
11. Make automatic local preparation and exact review the primary contribution
    journey. Keep manual file selection as an advanced recovery path and state
    the exact safe filename/contract; never ask for a raw Codex log.
12. Add community evidence coverage and freshness alongside community values,
    and separate the user's private comparison from cohort-wide telemetry.
13. Reduce visual hierarchy and page volume so the primary journey is: current
    allowance, estimated value of a full weekly allowance over time, measured
    versus calculated movement, and confidence. Detailed evidence remains
    available without dominating the page.
14. Verify the corrected real local page in a loopback browser at desktop and
    narrow widths, including chart labels, explanations, contribution
    onboarding, and the absence of console errors.
...
[2026-07-26-consumer-backend-real-data-verification-receipt.md](../receipts/2026-07-26-consumer-backend-real-data-verification-receipt.md).
Full-history R7 qualification, packaging, remote staging, and external
participant authorization remain open.

# July 27 corrective-pass verification

The corrective acceptance criteria above are implemented and verified locally.
The primary dashboard now reads a lineage-aware, replay-safe, content-free
accounting cache rather than treating copied child-rollout cumulative snapshots
as new usage.

For the seven-day window ending July 27, 2026, the corrected projection reports:

- 72,601 non-overlapping usage increments;
- 10,043,811,999 non-overlapping tokens;
- $6,282.48 of Standard-API-price-equivalent activity;
- 100% model-price coverage;
- 911,808 inherited child snapshots excluded across the cached scan window; and
- $1,906.71, or 30.3% of priced activity, attributed to genuine subagent
  increments rather than the invalid prior 91.5% raw-row share.

The consumer UI now uses cost for the primary activity view, allowance
remaining on a labeled 0–100% secondary axis, browser-local calendar grouping,
two-decimal consumer currency, explicit combined-output precedence, and a
permanent unrecognized/unpriced model overflow row. API service tier and
reasoning-effort settings are explained as unavailable rather than rendered as
misleading 100% unknown charts. Archived reports and manual safe-export file
selection are advanced disclosures; automatic local preparation is the primary
contribution route.

Cache freshness is checked independently from collector freshness. A stale
replay-safe cache is labeled stale rather than silently presented beside a
fresh collector timestamp. Refresh cancellation now propagates through the
lineage-aware log scan, and an aborted scan cannot publish a later cache.

Verification included desktop and 390-pixel responsive browser passes, local
timezone and dual-axis chart inspection, contribution file guidance,
community-contract telemetry fallbacks, zero console warnings/errors, 36
front-end tests, 64 loopback/local-product tests, 50 focused accounting/data
tests, and 22 normalization/scanner tests.
