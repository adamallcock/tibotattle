...
Actual public daily and allowance HTTP readers now use the central publication database. They preserve response schemas, suppression and caching, continue serving the prior complete cohort during an unrelated source failure, and refuse an obsolete erasure generation. A healthy ingestion shard accepts encrypted telemetry while another source is unavailable. The integrated eight-suite D1/HTTP check passes 96 tests; the broader public-reader selection passes 92 tests. Full second-checkpoint qualification remains separate.

Catalog deletion replay is still open. `retention.ts` (`deletionDigests`, `suppressRestoredParticipant`, `replayDeletionTombstones`, `runBackendLifecycle`) and the scheduled lifecycle caller in `index.ts` need a server-only immutable `participantDeletionDigest -> ownerId` locator recorded before erasure. It must enumerate exact route-history targets, recover restored participants through indexed owner links and match ledger jobs by source/namespace. It must not store raw participant IDs, scan all shards or infer completion from a missing owner. Root retains all hosted execution and the separately pinned recovery continuation.
