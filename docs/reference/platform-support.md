...
| Linux x86_64 | Portable/core and container checks may run; a packaged stable selection now fails closed at Electron entry before companion, credential, network, or updater composition | Contract or container results are not physical desktop qualification | No supported signed package/repository, clean install/uninstall receipt, updater boundary, or stable artifact | **Unsupported** |
