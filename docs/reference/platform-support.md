...
| Linux x86_64 | Portable/core and packaged native credential checks pass; an exact stable Linux/x64 source candidate may select the fixed credential composition, with mixed qualification/test lanes refused | Packaged startup, cold process restart with persisted settings/opt-out, disposable Secret Service and cleanup pass in Ubuntu container CI; physical desktop qualification remains open | AppImage production distribution, clean install/uninstall, current-to-next update acceptance and stable artifact remain open; no additional package/repository signing authority is required by the selected AppImage policy | **Unsupported** |
