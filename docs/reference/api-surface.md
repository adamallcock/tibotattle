...
| `GET` | `/api/local/model-performance` | Model performance page | Loopback read | Requires one `period` value: `1`, `7`, `30`, or `all`; optional canonical ISO `endAt` pins exact rolling bounds (never a future instant); returns bounded local timing aggregates, restores validated saved measurements before starting work, and renews an independent background scan lease. No accounting or network effect. | Local timing |
