# Usage Monitor — full product reference

> This is the complete historical product reference (formerly the repository
> README, relocated 2026-07-31). For an introduction and quick start, see the
...
The repository includes its own `pnpm-workspace.yaml`, with first-party shared
packages under `packages/*`, so pnpm does not inherit an unrelated workspace
from a parent directory. The policy also explicitly denies third-party
lifecycle builds for the digest-pinned Keytar prebuild. The independently
deployed Worker keeps its npm lockfile and installs shared packages as copied
`file:` dependencies; its check gate rejects a missing or stale installed copy.

### Repository architecture and quality gates

Root `src/` is the production local/domain implementation, not a collection of
one-off scripts. `apps/` contains runtime delivery surfaces, `packages/`
...
matches the canonical shared telemetry contract; regenerate it only with
`npm run telemetry:browser:generate`. `npm run check` runs the complete root
and product verification pipeline. Exact API-price-equivalent calculation and
price-registry provenance now have one public
`@app-usagemonitor/accounting` package boundary used by the local runtime and
Worker. The architecture baseline is empty: cross-surface inversions and any
attempt to reintroduce approved debt fail the architecture check.
...
Activity-history commands accept one to eight repeated `--codex-home PATH`
arguments and treat their logs as one local user history. Argument order has no
semantic effect. The loopback companion and native app additionally require one
configured root to be the primary root for live app-server and collector work;
history-only export and analysis commands do not. Secondary roots contribute
history only; they are not separate users, quota meters, contribution devices,
or upload credentials. Configure roots together only when they should be
treated as one user/account, because TiboTattle does not prove account
equivalence. Legacy live collector commands remain single-root.

- `usage-monitor doctor`: verify the selected local Codex binary and version,
  Codex app-server, ccusage, and RunCost integration. Binary selection is
  path-free in the diagnostic and follows runtime precedence: an explicit
  `CODEX_BIN`, ChatGPT's bundled Codex, Codex's bundled Codex, then `codex` on
  `PATH`. An unavailable version is reported as unavailable rather than being
  inferred from an app or package version.
...
- `npm run benchmark:r7:release:regenerate -- --node24 ABSOLUTE_PATH --node26 ABSOLUTE_PATH --start-at 2026-06-24T09:00:00.000Z --end-at 2026-07-25T09:00:00.000Z --destination ABSOLUTE_DIRECTORY --replace`: regenerate the complete retained ten-receipt release matrix only after every hashed source, contract, schema, package, and lockfile input is frozen. Run the wrapper itself under exact Node 26.2.0 on macOS arm64 and supply exact Node 24.14.0 and 26.2.0 child executables. It requires the exact preregistered 31-day interval, freezes one private source plan for both runtimes and both passes, stages and validates all ten content-free receipts, derives both decisions last, and publishes under an exclusive journal. If the owning process or machine stops, first verify that no regeneration process or restart job is still running, then rerun the wrapper under Node 26.2.0 with only `--destination ABSOLUTE_DIRECTORY --recover`; recovery refuses an exact live owner and otherwise either discards an incomplete staged generation, restores the prior complete matrix, or finishes a fully validated install. A competing invocation cleans only its own unpublished draft and does not require recovery of the active owner. An empty, truncated, or otherwise unverifiable pre-publication draft is retained for manual inspection and recovery fails closed without changing the retained receipts. Do not edit any hashed input after regeneration without regenerating again.
...
- `usage-monitor tools --since ISO_TIMESTAMP --until ISO_TIMESTAMP [--codex-home PATH ...]`: classify privacy-safe client tool observations separately from typed Responses/provider units and apply standard API tool prices only to an exact provider unit.
