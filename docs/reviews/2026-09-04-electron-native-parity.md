...
`6731e6d4` and the current Electron composition. It records implemented
...
| Foreground cadence uses a persisted 60/300/900/1800-second interval and rate-limits automatic detailed accounting to one attempt per hour in [`UsageMonitorApp.swift`](../../apps/macos/UsageMonitorApp.swift) (`NativeRefreshIntervalPreference`, `NativeDetailedRefreshCadence`, `scheduleNativeRefresh`). | [`apps/electron/desktop-controller.js`](../../apps/electron/desktop-controller.js) owns the persisted timer and lease; its closed `automaticRefresh` command carries `quick` or `detailed`, uses a monotonic in-process hourly reservation, and pauses while the dashboard is hidden. Manual `refresh` remains detailed; browser return refresh remains quick. | Source-complete for quick-first, bounded automatic mode selection and foreground gating. The last detailed attempt is not persisted across Electron process launches yet, unlike native `UserDefaults`. | Keep cross-launch cadence persistence as a separate qualification gate; run packaged timer smoke before claiming full native parity. |
...
| The menu-bar popover provides weekly pace and outlook from [`MenuBarPaceOutlook.swift`](../../apps/macos/Sources/MenuBarPaceOutlook.swift) and renders the control surface in [`MenuBarPopover.swift`](../../apps/macos/Sources/MenuBarPopover.swift). | Electron exposes the shared dashboard weekly view plus fixed `weekly` menu/tray actions through [`apps/web/public/app.js`](../../apps/web/public/app.js), [`desktop-menu.js`](../../apps/electron/desktop-menu.js), and [`desktop-tray.js`](../../apps/electron/desktop-tray.js). | Dashboard equivalent is rendered and verified in the synthetic packaged weekly screenshot (`.release-build/electron-unified-0.1.18-builds/1cfc6ba9/synthetic-weekly.png`), including 7-day/30-day/All controls, plan selection, and minimum observed-span disclosure; Electron has no native popover equivalent for the Swift presentation. | Keep the shared dashboard as the Electron control surface unless a separately scoped native popover is requested. |
| Native menu-bar history shows 7-day and 30-day horizons and retained-history state in [`MenuBarPopupModel.swift`](../../apps/macos/Sources/MenuBarPopupModel.swift) and [`MenuBarPopover.swift`](../../apps/macos/Sources/MenuBarPopover.swift). | Electron exposes shared dashboard weekly/timeline destinations and the same bounded history projections through [`desktop-shell.js`](../../apps/web/public/desktop-shell.js) and [`app.js`](../../apps/web/public/app.js). | Partial: dashboard navigation is present; this ledger does not claim that Electron currently exposes the native popover's retained-history disclosure. | Verify the packaged dashboard's history and coverage states, then add any missing disclosure before claiming popover parity. |
...
rendering or target-specific qualification.
