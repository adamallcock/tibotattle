...
import {
  ...
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_REF,
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_WORKFLOW,
  parseWindowsProductionFinalizerPreparationHandoff,
  serializeWindowsProductionFinalizerPreparationHandoff,
} from "./build-windows-production-finalizer-preparation-handoff.mjs";
...
export const WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_INPUT_FIXED_STATUS = Object.freeze({
  ...
  preparationInvalid: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_INPUT_BUILDER_PREPARATION_INVALID",
  preparationNoncanonical: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_INPUT_BUILDER_PREPARATION_NONCANONICAL",
  preparationMismatch: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_INPUT_BUILDER_PREPARATION_MISMATCH",
...
const CLI_FLAGS = Object.freeze(new Map([
  ...
  ["--selection", "selection"],
  ["--handoff", "handoff"],
  ["--preparation-handoff", "preparationHandoff"],
...
function validatePreparationHandoff(raw, handoff, finalizer) {
  let selected;
  try {
    selected = parseWindowsProductionFinalizerPreparationHandoff(raw.bytes.toString("utf8"));
  } catch {
    fail(STATUS.preparationInvalid);
  }
  assertCanonicalBytes(
    raw.bytes,
    selected,
    STATUS.preparationNoncanonical,
    serializeWindowsProductionFinalizerPreparationHandoff,
  );
  if (selected.source.revision !== handoff.revision
      || selected.source.ref !== handoff.run.ref
      || selected.qualification.revision !== handoff.revision
      || selected.qualification.run !== handoff.run.databaseId
      || selected.qualification.runAttempt !== handoff.run.runAttempt
      || selected.qualification.workflow !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_WORKFLOW
      || selected.workflow.path !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PREPARATION_WORKFLOW
      || selected.workflow.ref !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_REF
      || selected.workflow.revision !== handoff.revision
      || selected.workflow.run !== finalizer.run
      || selected.workflow.runAttempt !== finalizer.runAttempt) {
    fail(STATUS.preparationMismatch);
  }
  return selected;
}

function projectPreparationHandoff(preparation, sha256) {
  return Object.freeze({
    handoff: Object.freeze({ sha256 }),
    source: Object.freeze({
      ref: preparation.source.ref,
      revision: preparation.source.revision,
      run: preparation.qualification.run,
      runAttempt: preparation.qualification.runAttempt,
      workflow: preparation.qualification.workflow,
    }),
    workflow: Object.freeze({
      path: preparation.workflow.path,
      ref: preparation.workflow.ref,
      revision: preparation.workflow.revision,
      run: preparation.workflow.run,
      runAttempt: preparation.workflow.runAttempt,
    }),
  });
}
...
function validateInput(value) {
  ...
  const pathKeys = INPUT_KEYS.filter(
    (key) => key !== "evidenceRoot" && key !== "stagingRoot" && key !== "output",
  );
  const paths = Object.fromEntries(
    pathKeys.map((key) => [key, safeChildFilename(source[key], STATUS.inputInvalid)]),
  );
  const output = safeChildFilename(source.output, STATUS.inputInvalid);
  const folded = [...Object.values(paths), output].map((path) => path.toLowerCase());
  if (new Set(folded).size !== folded.length) fail(STATUS.inputInvalid);
  return Object.freeze({
    checkoutPackageJson: paths.checkoutPackageJson,
    evidenceRoot,
    finalizerMetadata: paths.finalizerMetadata,
    handoff: paths.handoff,
    preparationHandoff: paths.preparationHandoff,
    nativePresign: paths.nativePresign,
    output,
    policy: paths.policy,
    selection: paths.selection,
    sourceRunMetadata: paths.sourceRunMetadata,
    stagingRoot,
  });
}
...
export async function buildWindowsProductionFinalizerAuthorityInput(value, dependencies = {}) {
  ...
  const preparationRaw = await captureRegularFile(
    evidencePath(options.preparationHandoff),
    MAXIMUM_JSON_BYTES,
    STATUS.preparationInvalid,
    evidenceState,
  );
  ...
  const preparation = validatePreparationHandoff(preparationRaw, handoff, finalizer);
  ...
  try {
    presign = parseWindowsNativePresignReceipt(
      nativePresignRaw.bytes.toString("utf8"),
      {
        ...
        certificateSubjectSha256: policy.certificateSubjectSha256,
  ...
  const facts = Object.freeze({
    ...
    preparation: projectPreparationHandoff(preparation, preparationRaw.sha256),
  });
  const driverInput = buildDriverOptions({
    ...
    preparationHandoff: options.preparationHandoff,
...
function serializeDriverOptions(value) {
  ...
  safeChildFilename(source.preparationHandoff, STATUS.outputInvalid);
