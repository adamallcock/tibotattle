...
function validateTopLevel(value) {
  ...
  assertRawBuffer(
    source.preparationHandoffBytes,
...
function preparationFromHandoff({ preparation, preparationSha256, handoff, finalizer }) {
  if (preparation.source.revision !== handoff.revision
      || preparation.source.ref !== handoff.run.ref
      || preparation.qualification.run !== handoff.run.databaseId
      || preparation.qualification.runAttempt !== handoff.run.runAttempt
      || preparation.qualification.workflow !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_WORKFLOW
      || preparation.workflow.path !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PREPARATION_WORKFLOW
      || preparation.workflow.ref !== WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_REF
      || preparation.workflow.revision !== handoff.revision
      || preparation.workflow.run !== finalizer.run
      || preparation.workflow.runAttempt !== finalizer.runAttempt) {
    fail(WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_FIXED_STATUS.sourceRunMismatch);
  }
  return {
    handoff: { sha256: preparationSha256 },
    source: {
...
function projectNativeModules(receipt, publisher, certificateSubjectSha256) {
  ...
  return receipt.modules.map((module, index) => {
    ...
    if (module.authenticode?.publisher !== publisher
        || module.authenticode?.subjectSha256 !== certificateSubjectSha256) {
...
function buildAuthorityInput({
  ...
  if (presign.certificateSubjectSha256 !== certificateSubjectSha256) {
    fail(WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_FIXED_STATUS.presignLinkageMismatch);
  }
  const nativeModules = projectNativeModules(
    presign,
    publisher,
    presign.certificateSubjectSha256,
  );
  const nativePresign = {
    ...
    certificateSubjectSha256: presign.certificateSubjectSha256,
  };
...
export function buildWindowsProductionFinalizerAuthority(options) {
  try {
    ...
    const preparationHandoffBytes = Buffer.from(source.preparationHandoffBytes);
    const preparationHandoffSha256 = sha256(preparationHandoffBytes);
    const preparationText = decodeUtf8(
      preparationHandoffBytes,
      WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_FIXED_STATUS.handoffInvalid,
    );
    let preparationHandoff;
    try {
      preparationHandoff = parseWindowsProductionFinalizerPreparationHandoff(preparationText);
    } catch {
      fail(WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_FIXED_STATUS.handoffInvalid);
    }
    if (!Buffer.from(
      serializeWindowsProductionFinalizerPreparationHandoff(preparationHandoff),
      "utf8",
    ).equals(preparationHandoffBytes)) {
      fail(WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_FIXED_STATUS.handoffNoncanonical);
    }
    ...
    const authorityInput = buildAuthorityInput({
      handoff,
      handoffSha256,
      preparation: preparationFromHandoff({
        preparation: preparationHandoff,
        preparationSha256: preparationHandoffSha256,
        handoff,
        finalizer,
      }),
      presign,
