...
export const WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_DRIVER_FIXED_STATUS = Object.freeze({
  ...
  preparationInvalid: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_DRIVER_PREPARATION_INVALID",
  preparationNoncanonical: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_DRIVER_PREPARATION_NONCANONICAL",
  preparationMismatch: "WINDOWS_PRODUCTION_FINALIZER_AUTHORITY_DRIVER_PREPARATION_MISMATCH",
...
const MAXIMUM_PREPARATION_HANDOFF_BYTES = 256 * 1024;
...
const BUILDER_STATUS_TO_DRIVER_STATUS = Object.freeze([
  ["PREPARATION_NONCANONICAL", STATUS.preparationNoncanonical],
  ["PREPARATION_MISMATCH", STATUS.preparationMismatch],
  ["PREPARATION_", STATUS.preparationInvalid],
...
function validateFacts(value) {
  ...
  const certificateSubjectSha256 = assertSha256(
    source.certificateSubjectSha256,
    STATUS.factsInvalid,
  );
  ...
  const preparation = snapshotRecord(source.preparation, PREPARATION_KEYS, STATUS.factsInvalid);
  const preparationHandoff = snapshotRecord(
    preparation.handoff,
    PREPARATION_HANDOFF_KEYS,
    STATUS.factsInvalid,
  );
  const preparationSource = snapshotRecord(
    preparation.source,
    PREPARATION_SOURCE_KEYS,
    STATUS.factsInvalid,
  );
  const preparationWorkflow = snapshotRecord(
    preparation.workflow,
    PREPARATION_WORKFLOW_KEYS,
    STATUS.factsInvalid,
  );
  const normalizedPreparation = Object.freeze({
    handoff: Object.freeze({
      sha256: assertSha256(preparationHandoff.sha256, STATUS.factsInvalid),
    }),
    source: Object.freeze({
      ref: assertString(preparationSource.ref, /^refs\/heads\/main$/u, STATUS.factsInvalid, 128),
      revision: assertRevision(preparationSource.revision, STATUS.factsInvalid),
      run: assertPositiveInteger(preparationSource.run, STATUS.factsInvalid),
      runAttempt: assertPositiveInteger(preparationSource.runAttempt, STATUS.factsInvalid),
      workflow: assertString(
        preparationSource.workflow,
        new RegExp(`^${WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_WORKFLOW.replaceAll(".", "\\.")}$`, "u"),
        STATUS.factsInvalid,
        256,
      ),
    }),
    workflow: Object.freeze({
      path: assertString(
        preparationWorkflow.path,
        new RegExp(`^${WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PREPARATION_WORKFLOW.replaceAll(".", "\\.")}$`, "u"),
        STATUS.factsInvalid,
        256,
      ),
      ref: assertString(preparationWorkflow.ref, /^refs\/heads\/main$/u, STATUS.factsInvalid, 128),
      revision: assertRevision(preparationWorkflow.revision, STATUS.factsInvalid),
      run: assertPositiveInteger(preparationWorkflow.run, STATUS.factsInvalid),
      runAttempt: assertPositiveInteger(preparationWorkflow.runAttempt, STATUS.factsInvalid),
    }),
  });
...
export function validateWindowsProductionFinalizerAuthorityDriverOptions(value) {
  ...
  const preparationHandoff = assertRelativeEvidencePath(
    source.preparationHandoff,
    STATUS.optionsInvalid,
  );
...
function sameFacts(authority, facts) {
  ...
  return sameStableValue(authority.sourceQualification.binding, facts.filesystemBinding)
    ...
    && authority.nativePresign.certificateSubjectSha256 === facts.certificateSubjectSha256
    ...
    && sameStableValue(authority.preparation, facts.preparation);
}
...
export async function runWindowsProductionFinalizerAuthority(
  ...
  const [handoffBytes, preparationHandoffBytes, nativePresignBytes, checkoutPackageJsonBytes, sourceRunBytes] =
    await Promise.all([
      readEvidence(rootState, options.handoff, MAXIMUM_HANDOFF_BYTES, STATUS.handoffInvalid),
      readEvidence(
        rootState,
        options.preparationHandoff,
        MAXIMUM_PREPARATION_HANDOFF_BYTES,
...
async function optionsFromArguments(parsed) {
  ...
  const fixedPaths = [
    ...
    assertRelativeEvidencePath(values.preparationHandoff, STATUS.inputInvalid),
  ...
  return validateWindowsProductionFinalizerAuthorityDriverOptions({
    ...
    preparationHandoff: values.preparationHandoff,
