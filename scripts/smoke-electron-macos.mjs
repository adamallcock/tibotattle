...
async function runSmoke(appPath, progress = {}) {
  ...
  try {
    ...
    // These domains must be enabled before the page's first meaningful render
    // and before `localDashboardReady` is observed; otherwise the one startup
    // POST can be missed and a manual refresh would falsely pass the lane.
    ...
    stage = "settings";
    settingsReceipt = await assertSettingsFlow(cdp, port);
