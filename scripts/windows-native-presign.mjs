...
} from "./verify-windows-production-certificate-subject-preflight.mjs";
...
export const WINDOWS_NATIVE_PRESIGN_SUBJECT_HASH_ALGORITHM =
  WINDOWS_CERTIFICATE_SUBJECT_PREFLIGHT_ALGORITHM;
...
function scanJsonSyntax(text, code) {
  if (typeof text !== "string" || text.length === 0) fail(code);
  let index = 0;
  let nodes = 0;
  const whitespace = () => {
  ...
  const parseString = () => {
    ...
    while (index < text.length) {
      ...
      if (current === "\\") {
        index += 1;
        if (index >= text.length) fail(code);
...
function parseStrictJson(text, code, { allowBom = false } = {}) {
  if (typeof text !== "string" || Buffer.byteLength(text, "utf8") > MAXIMUM_INPUT_BYTES) {
    fail(code);
  }
  const source = allowBom ? text.replace(/^\uFEFF/u, "") : text;
  scanJsonSyntax(source, code);
  try {
...
export function validateWindowsNativePresignOptions(
  ...
  const certificateSubjectSha256 = assertString(
...
export function buildAuthenticodeProbeCommand(path) {
  ...
  return [
    ...
    "$subjectSha256 = -join ([System.Security.Cryptography.SHA256]::Create().ComputeHash($subjectBytes) | ForEach-Object { $_.ToString('x2') })",
    `& signtool.exe verify /pa /all ${literal} *> $null`,
    "$signtoolValid = ($LASTEXITCODE -eq 0)",
    "[ordered]@{ status = $signature.Status.ToString(); publisher = $publisher; signerThumbprint = $signature.SignerCertificate.Thumbprint; subjectSha256 = $subjectSha256; subjectUtf8Bytes = $subjectBytes.Length; timestampPresent = ($null -ne $signature.TimeStamperCertificate); policy = 'authenticode-pa'; signtoolPaValid = $signtoolValid } | ConvertTo-Json -Compress",
  ].join("; ");
}
...
function parseAuthenticodeProbe(stdout) {
  ...
  return parseStrictJson(
    stdout.replace(/^\uFEFF/u, "").trim(),
    WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid,
    { allowBom: false },
  );
}

export function validateAuthenticodeAggregate(
  value,
  expectedPublisher,
  expectedSubjectSha256,
) {
  // `publisher` is the certificate SimpleName returned by GetNameInfo, while
  // subjectSha256 binds the exact full distinguished Subject to the protected
  // Azure profile preflight. Raw Subject text is never accepted or retained.
  ...
  if (source.status !== "Valid"
      ...
      || typeof source.subjectSha256 !== "string"
      || !SHA256_PATTERN.test(source.subjectSha256)
      || source.subjectUtf8Bytes <= 0
      || !Number.isSafeInteger(source.subjectUtf8Bytes)
  ...
  if (typeof expectedSubjectSha256 !== "string"
      || !SHA256_PATTERN.test(expectedSubjectSha256)
      || source.subjectSha256 !== expectedSubjectSha256) {
    fail(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid);
  }
  return Object.freeze({
    status: "Valid",
...
function validateReceiptAuthenticode(
  value,
  expectedPublisher,
  expectedSubjectSha256,
) {
  // Receipts bind both the configured certificate SimpleName and the exact
  // preflight Subject hash. A full Subject DN is never serialized.
  const source = readRecord(
    value,
    AUTHENTICODE_KEYS,
    WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid,
  ...
  let subjectSha256;
  try {
    subjectSha256 = assertString(source.subjectSha256, SHA256_PATTERN, 64);
  } catch {
    fail(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid);
  }
  if (expectedSubjectSha256 !== undefined
      && (typeof expectedSubjectSha256 !== "string"
        || !SHA256_PATTERN.test(expectedSubjectSha256)
...
function validateExpectedReceiptBinding(value) {
  ...
  return Object.freeze({
    ...
    certificateSubjectSha256: assertString(
      expected.certificateSubjectSha256,
      SHA256_PATTERN,
      64,
    ),
  });
...
export function validateWindowsNativePresignReceipt(value, expectedBinding) {
  ...
  if (expected !== undefined
      && (revision !== expected.revision
        ...
        || certificateSubjectSha256 !== expected.certificateSubjectSha256)) {
    fail(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.inputInvalid);
  }
...
export function parseWindowsNativePresignReceipt(value, expectedBinding) {
  ...
  const parsed = parseStrictJson(value, WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.inputInvalid);
...
export async function buildWindowsNativePresignReceipt(
  ...
  for (const entry of unsigned) {
    ...
    const authenticode = validateAuthenticodeAggregate(
      ...
      options.certificateSubjectSha256,
    );
...
async function main(argv) {
  try {
    ...
    const input = parseStrictJson(text, WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.inputInvalid);
    if (text !== `${JSON.stringify(stableValue(input))}\n`) {
      fail(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.inputInvalid);
    }
