...
const CREDIT_MARKER = '"has_credits":true';
...
async function discoverRolloutFiles(root, since) {
...
function fileHasCredits(path) {
  return new Promise((resolvePromise, reject) => {
    ...
    stream.on("data", (chunk) => {
      ...
      if (window.includes(CREDIT_MARKER)) {
        hit = true;
        stream.destroy();
        return;
      }
      carry = window.slice(-CREDIT_MARKER.length);
    });
...
function settlementIncrements(records, creditUsd) {
  ...
  for (let i = 1; i < records.length; i += 1) {
    ...
    const slice = records.slice(anchor, i + 1);
    ...
    steps.push({
      ...
      ...sumComponents(slice),
      apiUsd,
      tailSuspect: apiUsd > 0 && usdDrawn / apiUsd > TAIL_SUSPECT_RATIO,
    });
    anchor = i;
  }
  return steps;
...
function analyzeEpisode(episode, index, creditUsd, bootstrap) {
  ...
  return {
    ...
    usedPercentRange: [
      Math.min(...records.map((record) => record.usedPercent ?? Number.POSITIVE_INFINITY)),
      Math.max(...records.map((record) => record.usedPercent ?? Number.NEGATIVE_INFINITY)),
    ],
...
function report(episodes, creditUsd) {
  ...
  for (const episode of episodes) {
    ...
    lines.push(`  models     ${episode.models.join(", ")}${episode.modelPure ? "" : "  (MIXED - per-model rates not attributable)"}`);
  ...
  if (table.length) {
    ...
    for (const row of table) {
      lines.push(`  ${row.model.padEnd(20)}${String(row.episodes).padStart(9)}${row.usdDrawn.toFixed(4).padStart(12)}${row.apiUsd.toFixed(4).padStart(12)}${row.ratio.toFixed(4).padStart(9)}${row.floors ? "  (floor)" : ""}`);
    }
    if (table.length > 1) {
      const base = table[0];
      lines.push("");
      lines.push(`  relative to ${base.model}:`);
      for (const row of table.slice(1)) {
        lines.push(`    ${row.model.padEnd(20)} ${(row.ratio / base.ratio).toFixed(4)}x`);
      }
    } else {
      lines.push("");
  ...
  for (const episode of episodes) {
    for (const record of episode.records) {
      rows.push([
        ...
        record.planType ?? "", record.usedPercent ?? "", record.sourceFile,
      ].join(","));
    }
  }
  return rows.join("\n");
...
`);
  ...
  if (targets.length === 0) {
    const candidates = await discoverRolloutFiles(opts.sessions, opts.since);
  ...
  const records = [];
  for (const target of targets) records.push(...await readCreditRecords(target));
  ...
  if (opts.json) {
    const payload = {
      ...
      sourceFiles: targets,
      episodes: analyzed.map(({ records: _records, ...rest }) => rest),
  ...
  if (opts.csv) {
    await writeFile(opts.csv, `${toCsv(analyzed)}\n`);
