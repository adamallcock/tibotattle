...
- `usage-monitor inspect-export --since ISO_TIMESTAMP --until ISO_TIMESTAMP [--codex-home PATH] [--activity-file PATH] [--secret-file PATH]`: construct and privacy-check one resource-bounded metadata-only bundle, then print only coverage, counts, resource-policy version, check results, and size. It never writes a bundle or contacts a server. Histories that exceed a ceiling fail explicitly; deterministic export-set splitting/resume is the next G1-R3 implementation and is not yet available.
...
The export privacy gate validates the complete bundle schema, requires the live compatibility tuple and an implemented adapter for every declared or observed provider, recursively scans forbidden keys and sensitive string shapes outside the closed protocol tuple, verifies record counts, and emits a SHA-256 receipt. The scanner bounds allocation for one JSONL line at 16 MiB. A fixed-marker streaming classifier may discard an oversized line only when it proves the line cannot be a session, context, tier, usage, task, or tool record; a relevant or ambiguous oversized line fails closed while physical line ordinals remain stable. Export discovery/runtime/output uses a versioned candidate policy: 31 covered days, 20,000 traversed directory entries, 5,000 source files, 32 GiB discovered source bytes, 100,000 output records, 32 MiB expanded safe-record bytes/canonical bundle bytes, ten elapsed minutes, and a 1.5 GiB RSS kill switch. The same guard covers optional activity-marker reading and bundle construction. Those are preliminary single-machine safety ceilings, not final p95 volunteer limits. The standalone verifier enforces the 32 MiB/100,000-record serialized side before independently checking exact canonical bytes and semantic invariants. Source-value canaries remain fixture/export-time evidence only when explicitly supplied; the verifier cannot reconstruct discarded raw source values. Any failure stops or rejects the export without echoing private values. `exports/`, `*.umx`, and privacy receipts are ignored by Git as a second local safeguard.

This is not yet a sharing mechanism. There is no upload client, server, enrollment code, encryption envelope, background process, or public dashboard. The [G1 resource-bounded export-set plan](./2026-07-24-g1-resource-bounded-export-set-plan.md) defines the active local milestone, and the [resource/identity verification receipt](./2026-07-24-g1-resource-identity-protection-receipt.md) records the completed bounded slice and its remaining gaps. The [complete end-to-end goal](./2026-07-24-end-to-end-multi-user-usage-monitor-goal.md) defines the critical path and production finish criteria; the [multi-user privacy expansion plan](./2026-07-24-multi-user-privacy-expansion-plan.md) contains the supporting architecture.
