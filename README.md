...
- `usage-monitor export-set --workspace PATH --directory PATH [--resume] [--since ISO_TIMESTAMP --until ISO_TIMESTAMP] [--codex-home PATH] [--activity-file PATH] [--secret-file PATH] [--max-records-per-chunk N] [--max-bundle-bytes N] [--max-artifact-bytes N]`: freeze complete Codex source prefixes, stream validated safe records through an owner-only SQLite workspace, resume an interrupted scan without duplicate logical records, materialize deterministic bounded gzip bundle/receipt chunks, and publish the complete manifest last. The decoded canonical and encoded artifact ceilings are independent. Creation requires `--since` and `--until`; `--resume` rejects new bounds and binds to the original identity, compatibility tuple, interval, source plan, normalized activity-marker digest, and creation time. Upload remains absent.
