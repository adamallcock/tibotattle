...
- `usage-monitor benchmark-r7 --profile smoke --output PATH`: run two isolated passes of the content-free synthetic resource/lifecycle harness and write an owner-only, self-hashed receipt. Only the smoke profile exists; `release` is refused until the preregistered near-limit and real heavy-history workload is implemented. The command does not upload data and reports network activity as unmeasured.
