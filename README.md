...
- `usage-monitor export-local --since ISO_TIMESTAMP --until ISO_TIMESTAMP --output PATH [--receipt PATH] [--codex-home PATH] [--activity-file PATH] [--secret-file PATH]`: write the same validated bundle and its SHA-256 privacy receipt as a no-clobber owner-only pair in one destination directory. The receipt is published first and the bundle last as the commit point; an ordinary second-link failure rolls both back. The v0.1 bundle is forced to `transportReady: false`; no upload command, endpoint, or network transport is implemented.
