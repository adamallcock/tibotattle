...
- `usage-monitor export-set --workspace PATH --directory PATH [--resume] [--since ISO_TIMESTAMP --until ISO_TIMESTAMP] [--codex-home PATH] [--activity-file PATH] [--secret-file PATH] [--max-records-per-chunk N] [--max-bundle-bytes N]`: freeze complete Codex source prefixes, stream validated safe records through an owner-only SQLite workspace, resume an interrupted scan without duplicate logical records, materialize deterministic bounded bundle/receipt chunks, and publish the complete manifest last. Creation requires `--since` and `--until`; `--resume` rejects new bounds and binds to the original identity, compatibility tuple, interval, source plan, normalized activity-marker digest, and creation time. Upload remains absent.
...
- `usage-monitor verify-export-set --directory PATH`: independently verify the canonical manifest and its receipt, every inferred fixed-name chunk pair, exact hashes/bytes/counts/shared contract, cross-chunk ordering and occurrence-ID uniqueness using a temporary disk-backed index, greedy packing boundaries, and the chunk-independent logical-record digest. Output is content-free.
...
- `usage-monitor rotate-local-identity [--secret-file PATH] [--confirm]`: without confirmation, report only `ready`, `missing`, `external_override`, or `conflict` and change nothing. With `--confirm`, atomically replace a canonical owner-only file identity, retire legacy fallback, and break linkability for every future participant/session/event/snapshot/account/model pseudonym. Existing bundles are not changed; environment-provided secrets are refused; secure SSD erasure is not claimed; no network activity occurs.
...
Telemetry v0.1 is explicitly an unfrozen, local-only draft: it has never authorized external participants or transport, and old local review artifacts have no compatibility promise and must be regenerated after a schema-hash change. `contracts/telemetry-v0.1/contract-status.json` and `contracts/telemetry-v0.1/consent-status.json` make that state machine-checkable. The generated compatibility tuple hashes all six schemas, the generated field dictionary, reviewed registry snapshot, consent status, and contract status; it also pins the package/exporter and executed Codex parser/adapter versions. Claude is explicitly `not_implemented`, so a Claude-declaring bundle fails closed. The first volunteer or upload-capable contract will use a new frozen version; once frozen, a schema version is immutable and all changes require a new version plus migration tests.

Allowed data is limited to exact event/snapshot timestamps, available disjoint cached/uncached/output/reasoning token counts, recognized model IDs or opaque model fingerprints, fixed subscription speed and API tier enums, fixed surface/agent/lineage classes, coarse tool-class counts, quota percentages/window/reset timing, plan classes, and domain-separated participant/session/event/snapshot/account pseudonyms. Missing source token components remain `null`, never an invented observed zero. Prompt and response content, tool names and arguments, commands, URLs, paths, repositories, filenames, branches, account emails, raw account/session/device identifiers, credentials, hostnames, usernames, arbitrary labels, and unknown fields are excluded.

The separate exporter secret defaults to the OS application-state directory: `~/Library/Application Support/app-usagemonitor/export-participant-secret` on macOS, `${XDG_STATE_HOME:-~/.local/state}/app-usagemonitor/export-participant-secret` on Linux, or `%LOCALAPPDATA%/app-usagemonitor/export-participant-secret` on Windows. A safe legacy working-directory secret is copied forward once, then an owner-only retirement tombstone prevents it from resurrecting an older identity. Canonical/legacy disagreement fails closed instead of silently choosing one. POSIX file fallback requires a single-link regular file, current-user ownership, exact length, an owner-controlled directory, and mode `0600`; native Windows credential storage remains a release blocker. `APP_USAGEMONITOR_EXPORT_SECRET` and `--secret-file` remain advanced overrides. The secret is intentionally distinct from the account-observation HMAC key.

Usage and quota occurrence IDs are keyed to the privacy-safe source-session scope plus the physical JSONL record ordinal, so parser enrichment, model-registry changes, tool attribution, and export bounds do not re-key the source event. Quota records also carry a separate provider-state ID; unattributed state is session-scoped so two unknown accounts are never collapsed. Reviewed model, limit, and diagnostic vocabularies fail closed. `pnpm telemetry:check` verifies that every one of the 149 properties across all six schemas has exactly one reviewed privacy/retention/publication/provider-provenance row in `generated/telemetry-v0.1-field-dictionary.json`, that all references resolve, and that both the field dictionary and compatibility manifest match the live checked-in inputs. The compatibility tuple also binds the candidate G1 resource-policy version.

The export privacy gate validates the complete bundle schema, requires the live compatibility tuple and an implemented adapter for every declared or observed provider, recursively scans forbidden keys and sensitive string shapes outside the closed protocol tuple, verifies record counts, and emits a SHA-256 receipt. The scanner bounds allocation for one JSONL line at 16 MiB. A fixed-marker streaming classifier may discard an oversized line only when it proves the line cannot be a session, context, tier, usage, task, or tool record; a relevant or ambiguous oversized line fails closed while physical line ordinals remain stable. Frozen prefixes are checked before reading and rehashed on the same open descriptor after parsing before scan completion. Export discovery/runtime/output uses versioned candidate policy `g1-r3-candidate-0.3`: 31 covered days, 20,000 traversed directory entries, 5,000 source files, 32 GiB frozen source bytes, 2,000,000 safe records/2 GiB expanded safe-record bytes per set, 100,000 records/32 MiB canonical bytes per chunk, 512 chunks, a 1 MiB manifest, a 4 GiB workspace or verifier uniqueness index, 1,000-row SQLite transactions, ten elapsed minutes per invocation, and a 1.5 GiB RSS kill switch. Those are preliminary single-machine safety ceilings, not final p95 volunteer limits. The single-bundle and set verifiers independently enforce their serialized and semantic boundaries. Source-value canaries remain fixture/export-time evidence only when explicitly supplied; a verifier cannot reconstruct discarded raw source values. Any failure stops or rejects the export without echoing private values. `exports/`, `*.umx`, privacy receipts, and `.usage-monitor/` state are ignored by Git as local safeguards.

The export-set path requires a Node runtime exposing `node:sqlite`; it is tested here on Node 26.2.0. Older supported runtimes can continue using the legacy local commands, but are not release-qualified for export sets. This is still not a sharing mechanism: there is no upload client, server, enrollment code, encryption envelope, background process, or public dashboard. The [G1 resource-bounded export-set plan](./2026-07-24-g1-resource-bounded-export-set-plan.md) defines the active local milestone, the [disk-backed export-set verification receipt](./2026-07-24-g1-disk-backed-export-set-receipt.md) records the current verified foundation and open blockers, and the [resource/identity verification receipt](./2026-07-24-g1-resource-identity-protection-receipt.md) records the earlier bounded slice. The [complete end-to-end goal](./2026-07-24-end-to-end-multi-user-usage-monitor-goal.md) defines the critical path and production finish criteria; the [multi-user privacy expansion plan](./2026-07-24-multi-user-privacy-expansion-plan.md) contains the supporting architecture.
