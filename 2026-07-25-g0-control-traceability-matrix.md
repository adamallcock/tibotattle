...
| C-03 | Official client never transmits content | `transportReady: false` remains fixed for local telemetry v0.1; the separately versioned development contribution builder/browser sends only a reverified closed metadata projection after explicit action | `test/export-contract.test.js`, `test/export-privacy.test.js`, `test/activity-markers.test.js`, `test/telemetry-contribution-builder.test.js`, `apps/web/test/lib.test.mjs`, `apps/worker/test/worker.spec.ts` | [G1 multi-source receipt](./2026-07-24-g1-multi-source-local-export-verification-receipt.md), [functional E2E receipt](./2026-07-25-functional-product-e2e-verification-receipt.md) | Local v0.1 source claim implemented; development transport proven; packaged no-egress and frozen successor contract remain open |
...
| C-09 | Personal and public products use different disclosure rules | Local/private dashboard and participant APIs are separated from a thresholded fixed community API; invite-only aggregates require distinct grant-backed eligibility relations; no arbitrary aggregate query or public route | `test/telemetry-contract.test.js`, `test/export-contract.test.js`, `apps/web/test/lib.test.mjs`, `apps/worker/test/worker.spec.ts` | [Complete goal](./2026-07-24-end-to-end-multi-user-usage-monitor-goal.md), [G3 invitation receipt](./2026-07-25-g3-invite-admission-verification-receipt.md) | Development separation and eligibility implemented; clipping, disclosure review, filter lattice, and public release remain blocking |
| C-10 | Server cannot broaden local collection | Explicit CLI source selection and bound source plans; loopback/central APIs accept only fixed routes and prepared closed contributions, never source paths or server-directed scan commands | `test/export-set-controller.test.js`, `test/export-supplemental-source-plan.test.js`, `test/supplemental-source-lifecycle.test.js`, `apps/local/server.test.mjs`, `apps/worker/test/worker.spec.ts` | [G1 multi-source receipt](./2026-07-24-g1-multi-source-local-export-verification-receipt.md), [local companion plan](./2026-07-25-local-companion-app-plan.md) | Implemented for the manual development slice; signed client/update policy and ongoing collection remain open |
