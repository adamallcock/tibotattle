...
test("unknown credit expiry cannot prove a pre-expiry banked reset", () => {
  for (const detailsStatus of ["complete", "partial", "unavailable"]) {
    const classifier = createResetEventClassifier();
    classifier.observe(observation("2026-09-10T00:00:00.000Z", 88, {
      resetCredits: inventory(1, [{
        id: "reset-credit:v1:unknown_expiry",
        grantedAt: null,
        expiresAt: null,
      }]),
    }));
    assert.deepEqual(classifier.observe(observation(
      "2026-09-10T01:00:00.000Z", 4, {
        resetCredits: inventory(0, [], detailsStatus),
      },
    )), [], `${detailsStatus} detail evidence cannot turn unknown expiry into use`);
    assert.deepEqual(classifier.observe(observation(
      "2026-09-10T01:05:00.000Z", 5,
    )).map((event) => event.kind), ["unknown_reset"],
    "a confirmed quota drop still remains visible with uncertain cause");
  }
});

test("count-only evidence must exceed every possibly expired credit", () => {
  for (const expiresAt of [null, "2026-09-09T00:00:00.000Z"]) {
    for (const remainingCount of [0, 1]) {
      const classifier = createResetEventClassifier();
      classifier.observe(observation("2026-09-10T00:00:00.000Z", 88, {
        resetCredits: inventory(2, [{
          id: "reset-credit:v1:possibly_expired",
          grantedAt: null,
          expiresAt,
        }, {
          id: "reset-credit:v1:known_unexpired",
          grantedAt: null,
          expiresAt: "2026-10-01T00:00:00.000Z",
        }]),
      }));
      const events = classifier.observe(observation(
        "2026-09-10T01:00:00.000Z", 4, {
          resetCredits: inventory(remainingCount, [], "unavailable"),
        },
      ));
      assert.deepEqual(events.map((event) => event.kind), remainingCount === 0
        ? ["banked_reset_used"]
        : [], "one potentially expired credit can explain only one removal");
    }
  }
});
