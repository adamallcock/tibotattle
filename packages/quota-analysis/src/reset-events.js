...
export function createResetEventClassifier() {
  ...
  return Object.freeze({
    ...
    observe(value) {
      ...
      // A missing expiry is not evidence that a credit could not have expired.
      // Without current detail rows, every credit not known to remain unexpired
      // can explain the count fall; only a larger fall proves some other removal.
      const possiblyExpiredCount = removedCredits.length - unexpiredRemoved.length;
      const provesUnexpiredRemoval = creditContinuityFresh
        && countDrop > 0
        && priorCredits.detailsStatus === "complete"
        && (current.resetCredits.detailsStatus === "complete"
          ? unexpiredRemoved.length > 0
          : possiblyExpiredCount < countDrop);
