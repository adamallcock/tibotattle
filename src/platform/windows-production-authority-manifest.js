...
function validatePreparation(value, sourceQualification, finalizer, sourceRevision) {
  const preparation = readRecord(value, PREPARATION_KEYS);
  const handoff = readRecord(preparation.handoff, PREPARATION_HANDOFF_KEYS);
  const source = readRecord(preparation.source, PREPARATION_SOURCE_KEYS);
  const workflow = readRecord(preparation.workflow, PREPARATION_WORKFLOW_KEYS);
  const handoffSha256 = assertSha256(handoff.sha256);
  const sourceRef = assertRef(source.ref);
  const sourceRevisionValue = assertRevision(source.revision);
  const sourceRun = assertPositiveInteger(source.run);
  const sourceRunAttempt = assertPositiveInteger(source.runAttempt);
  const sourceWorkflow = assertWorkflow(source.workflow);
  const workflowPath = assertWorkflow(workflow.path);
  const workflowRef = assertRef(workflow.ref);
  const workflowRevision = assertRevision(workflow.revision);
  const workflowRun = assertPositiveInteger(workflow.run);
  const workflowRunAttempt = assertPositiveInteger(workflow.runAttempt);

  if (sourceRef !== sourceQualification.ref
      || sourceRevisionValue !== sourceRevision
      || sourceRun !== sourceQualification.run
      || sourceRunAttempt !== sourceQualification.runAttempt
      || sourceWorkflow !== sourceQualification.workflow
      || workflowPath !== finalizer.workflow
      || workflowRef !== finalizer.ref
      || workflowRevision !== sourceRevision
      || workflowRun !== finalizer.run
      || workflowRunAttempt !== finalizer.runAttempt) {
    fail("mismatch");
  }
...
function validateNativePresign(value, sourceRevision, packageVersion, handoffSha256) {
  ...
  const certificateSubjectSha256 = assertSha256(presign.certificateSubjectSha256);
