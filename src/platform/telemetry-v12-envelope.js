...
/** Same reviewed RSA-OAEP/AES-GCM construction; v1.0/v1.1 bytes stay untouched. */
export async function createTelemetryV12Envelope({
  ...
  let plaintext;
  try { plaintext = new TextEncoder().encode(JSON.stringify(parseTelemetryV12Chunk(chunk))); }
  ...
  try {
    ...
    return Object.freeze(validateTelemetryV12Envelope({
      schemaVersion: TELEMETRY_V12_ENVELOPE_SCHEMA_VERSION, synthetic: false, keyId,
