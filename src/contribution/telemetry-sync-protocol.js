...
import { accountlessDeviceUnavailableCode, accountlessTransportOrigin } from "./accountless-transport-contract.js";

// Only reviewed static dialects construct this closure; callers cannot override
// versions, parsers, routes or authorization through per-run options.
export function createTelemetrySyncProtocol(contract) {
...
const PROGRESS_VERSION = contract.progressVersion;
...
class SyncFailure extends Error {
  constructor(code, { retryable = false, deviceUnavailable = false, retryAfterMilliseconds = null } = {}) {
    ...
    this.name = contract.failureName;
...
function capabilities(value, origin) {
  if (contract.capabilitiesKind === "successor") {
    if (!exact(value, ["schemaVersion", "destinationOrigin", "enrollmentNamespace",
      "identityVersion", "authorityKind", "successor"])
        || value.schemaVersion !== contract.capabilitiesVersion || value.destinationOrigin !== origin
        ...
        || !exact(value.successor, ["schemaVersion", "envelopeSchemaVersion", "lifecycle",
          "requiredConsent", "consentCurrent", "authorizationCurrent", "activationTime"])
        || value.successor.schemaVersion !== contract.contributionSchemaVersion
        || value.successor.envelopeSchemaVersion !== contract.envelopeSchemaVersion
        || !["accepted", "staged", "blocked"].includes(value.successor.lifecycle)
        || !contract.isConsentCurrent(value.successor.requiredConsent)
        || typeof value.successor.consentCurrent !== "boolean"
        || typeof value.successor.authorizationCurrent !== "boolean"
        || (value.successor.activationTime !== null && !instant(value.successor.activationTime))
        || (value.successor.authorizationCurrent && value.successor.activationTime === null)
        || (value.authorityKind === "accountless" && value.successor.consentCurrent)
        || (value.successor.authorizationCurrent && (value.successor.lifecycle !== "accepted"
          || (value.authorityKind === "social" && !value.successor.consentCurrent)))) stop("response_invalid");
    // Normalize only inside this transport engine. The public capability reader
    // returns the validated wire representation unchanged.
    return freeze({ schemaVersion: value.schemaVersion, destinationOrigin: value.destinationOrigin,
      enrollmentNamespace: value.enrollmentNamespace, identityVersion: value.identityVersion,
      ...(value.authorityKind === "accountless" ? { authorityKind: "accountless",
        authorizationCurrent: value.successor.authorizationCurrent } : {}),
      consentCurrent: value.successor.consentCurrent && value.successor.authorizationCurrent,
      requiredConsent: value.successor.requiredConsent, minimumWriteRank: 12,
      activationTime: value.successor.activationTime,
      policyRevision: hash(JSON.stringify([value.successor.requiredConsent, value.successor.activationTime])),
      formats: [{ schemaVersion: value.successor.schemaVersion, rank: 12,
        lifecycle: value.successor.lifecycle }],
    });
  }
  ...
  if (!exact(value, keys)
      ...
      || !contract.isConsentCurrent(value.requiredConsent)
      || !Array.isArray(value.formats) || value.formats.length !== Object.keys(FORMATS).length
      || !sanitizeTelemetryAttributionBinding({ destinationOrigin: value.destinationOrigin,
...
function requireTransportAdmission(capability, accountless) {
  const format = capability.formats.find((item) => item.schemaVersion === contract.contributionSchemaVersion);
...
function accountlessAuthorization(value, laboratory, rehearsal, production, serverBaseUrl) {
  if (accountlessTransportOrigin({ laboratory, rehearsal, production, origin: serverBaseUrl }) === null
      ...
      || value.schemaVersion !== contract.accountlessSchemaVersion
      || value.policyVersion !== contract.accountlessPolicyVersion
      || value.authorizationBasis !== contract.accountlessAuthorizationBasis
      || value.telemetrySchemaVersion !== contract.contributionSchemaVersion) {
...
// The exact dialect dictionary stays on the wire for both modes. In the
...
function transportAuthorization({ consent, authorization, laboratory, rehearsal, production, serverBaseUrl }) {
  ...
  if (!contract.isConsentCurrent(consent)) {
...
async function readCapabilities({
  ...
  try {
    const value = await client.request(contract.capabilitiesPath);
    const validated = capabilities(value, client.origin);
    return contract.capabilitiesKind === "successor" ? freeze(value) : validated;
  }
  catch (error) {
    if (error instanceof SyncFailure) throw error;
    stop("service_unavailable", { retryable: true });
...
function predecessor(value, clock) {
  if (!exact(value, ["schemaVersion", "token", "previousGenerationId", "legacyFingerprint", "fromDay", "throughDay", "expiresAt"])
      || value.schemaVersion !== contract.predecessorVersion
...
function snapshotDay(value, day) {
  try {
    if (!exact(value, ["manifest", "chunks"]) || !Array.isArray(value.chunks)
        || value.chunks.length > contract.maximumDayChunks) stop("index_unavailable", { retryable: true });
    const manifest = JSON.parse(contract.canonicalJson(contract.parseDayManifest(value.manifest)));
    if (manifest.day !== day || manifest.chunks.length !== value.chunks.length
        || hash(contract.dayManifestDigestInput(manifest)) !== manifest.manifestDigest) stop("local_index_changed", { retryable: true });
    let byteCount = 0;
    const seen = { quota: new Set(), session: new Set(), usage: new Set() };
    const chunks = [];
    for (let i = 0; i < value.chunks.length; i += 1) {
      const serialized = contract.canonicalJson(contract.parseChunk(value.chunks[i]));
      ...
      if (chunk.manifestDigest !== manifest.manifestDigest || chunk.parserVersion !== manifest.parserVersion
          ...
          || hash(contract.canonicalJson(chunk.records)) !== chunk.chunkDigest) stop("local_index_changed", { retryable: true });
      const { stream } = contract.parseChunkId(chunk.chunkId);
      for (const record of chunk.records) {
        const id = contract.recordAnchor(stream, record).occurrenceId;
        if (seen[stream].has(id)) stop("index_unavailable");
        seen[stream].add(id);
      }
      chunks.push(chunk);
    }
    if (contract.validateUsageDay) contract.validateUsageDay(day, chunks
      .filter((chunk) => contract.parseChunkId(chunk.chunkId).stream === "usage")
...
function chunkReceipt(value, chunk, candidate) {
  if (!exact(value, ["schemaVersion", "contributionId", "manifestId", "chunkId", "chunkRevision", "status", "replayed", "recordCounts"])
      || value.schemaVersion !== contract.chunkReceiptVersion
...
function activation(value, manifest) {
  ...
  if ((!exact(value, commonKeys) && !unchanged) || value.schemaVersion !== contract.activationVersion
...
function progressContext(publication, capability, deviceAuthorization, journalBinding) {
  ...
  return hash(contract.canonicalJson(context));
}
...
async function runSync({
  ...
  maxDays = contract.maximumDomainDays,
  ...
  if (!integer(maxChunks, 2_000) || maxChunks < 1 || !integer(maxDurationMs, 300_000) || maxDurationMs < 1
      || !integer(maxDays, contract.maximumDomainDays) || maxDays < 1
  ...
  try {
    const capability = capabilities(await client.request(contract.capabilitiesPath), client.origin);
    ...
    const projectionContext = Object.freeze({ binding, ...(contract.capabilitiesKind === "successor"
      ? { activationTime: capability.activationTime } : {}) });
    ...
    if (preparePublication !== null) {
      try { publication = publicationSnapshot(await client.bounded(() => preparePublication(projectionContext))); }
      catch (error) {
        if (error instanceof SyncFailure || error instanceof PassBudgetReached) throw error;
    ...
    let before = predecessor(await client.request(contract.predecessorPath, { body: {} }), clock);
    ...
    async function prepareDay(day) {
      let prepared;
      try { prepared = snapshotDay(await client.bounded(() => readDay(day, projectionContext)), day); }
      catch (error) {
        if (error instanceof SyncFailure || error instanceof PassBudgetReached) throw error;
    ...
    for (let i = vector.length; i < count; i += 1) {
      ...
      changedDay = null;
      const candidate = dayCandidate(await client.request(contract.dayManifestsPath,
      ...
      for (const chunk of prepared.chunks) {
        ...
        try {
          ...
          body = JSON.stringify(contract.validateEnvelope(envelope));
        } catch (error) {
          if (error instanceof SyncFailure || error instanceof PassBudgetReached) throw error;
          stop("index_unavailable", { retryable: true });
        }
        const authorization = await client.request("/api/v1/device/upload-authorizations", { body: {
          ...
          contentType: "application/json", telemetrySchemaVersion: contract.contributionSchemaVersion,
        } });
    ...
    const after = capabilities(await client.request(contract.capabilitiesPath), client.origin);
    ...
    if (progressStore !== null || Date.parse(before.expiresAt) <= clock() + requestTimeoutMs) {
      const renewed = predecessor(await client.request(contract.predecessorPath, { body: {} }), clock);
    ...
    const manifest = {
      schemaVersion: contract.domainManifestSchemaVersion, fromDay, throughDay,
    ...
    manifest.manifestDigest = hash(contract.domainManifestDigestInput(manifest));
    contract.parseDomainManifest(manifest);
    active = activation(await client.request(contract.activationPath, { body: manifest }), manifest);
    if (progressStore !== null) {
...
return Object.freeze({ readCapabilities, runSync });
}
