...
function normalizedComponents(record) {
  return {
    inputUncachedTokens: nullableComponent(
      record.components,
      "inputUncachedTokens",
    ),
    inputCacheReadTokens: nullableComponent(
      record.components,
      "inputCacheReadTokens",
    ),
    inputCacheWriteTokens: nullableComponent(
      record.components,
      "inputCacheWriteTokens",
    ),
    inputCacheWrite5mTokens: nullableComponent(
      record.components,
      "inputCacheWrite5mTokens",
    ),
    inputCacheWrite1hTokens: nullableComponent(
      record.components,
      "inputCacheWrite1hTokens",
    ),
    outputTextTokens: nullableComponent(record.components, "outputTextTokens"),
    outputReasoningTokens: nullableComponent(
      record.components,
      "outputReasoningTokens",
    ),
    outputCombinedTokens: nullableComponent(
      record.components,
...
function unknownBillableUnits(result) {
  return Array.isArray(result?.components)
    ? result.components.filter((component) =>
      component?.pricingStatus === "unpriced").length
    : 0;
}
...
function transportUsage(record) {
  ...
  return {
    ...
    components,
    totalInputContextTokens: Number.isSafeInteger(
    ...
    accounting: {
      estimatedApiCostUsd: fullyUnpriced
        ? null
...
function batchAccounting(usageEvents) {
  ...
  return {
    ...
    unknownModelEventCount: usageEvents.filter((row) =>
      row.modelId === "unknown").length,
...
function assertTransportProjection(value) {
  ...
  if (/(?:accountScopeId|sessionScopeId|participantId|providerStateId)/u
    .test(serialized)) {
    throw fixedError("private_scope_leaked");
  }
  if (/(?:\/Users\/|\/home\/|file:\/\/|https?:\/\/|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,})/iu
    .test(serialized)) {
...
export function buildTelemetryContributionsFromBundle(bundle) {
  ...
  if (batch.usageEvents.length
      + batch.quotaSnapshots.length
      + batch.activityMarkers.length > 0) {
    batches.push(batch);
  }
