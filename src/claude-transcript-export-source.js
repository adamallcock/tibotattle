...
import { deriveSessionScopeId } from "./export-identity.js";
import { safeExportModelDeclaration } from "./export-safe-records.js";
import { ExportResourceLimitError } from "./export-resource-policy.js";
import { stableJson } from "./storage.js";

export const CLAUDE_TRANSCRIPT_SOURCE_PLAN_VERSION = "claude-transcript-export-source-plan-v0.2";
export const CLAUDE_TRANSCRIPT_SOURCE_CURSOR_VERSION = "claude-transcript-export-cursor-v0.2";
export const CLAUDE_TRANSCRIPT_USAGE_CANDIDATE_VERSION = "claude-transcript-usage-candidate-v0.2";
...
export class ClaudeTranscriptExportSourceError extends Error {
  constructor(code) {
...
function normalizeSecret(secret) {
  if (!Buffer.isBuffer(secret) && !(secret instanceof Uint8Array)) fail("configuration");
  const copy = Buffer.from(secret);
  if (copy.byteLength !== 32) fail("configuration");
  return copy;
}

export function defaultClaudeProjectsDirectory({ homeDirectory = homedir() } = {}) {
...
function assertSafeDirectory(stats) {
  if (!stats.isDirectory() || stats.isSymbolicLink()
      ...
      || (process.platform !== "win32" && (stats.mode & 0o022) !== 0)) fail("root_unsafe");
}

function assertSafeFile(stats) {
  if (!stats.isFile() || stats.isSymbolicLink() || stats.nlink !== 1
      || (typeof process.getuid === "function" && stats.uid !== process.getuid())
      || (process.platform !== "win32" && (stats.mode & 0o022) !== 0)) fail("source_unsafe");
}
...
function rowIdentity(record, secret) {
  ...
  if (!message.usage || typeof message.usage !== "object" || Array.isArray(message.usage)
      ...
      || Buffer.byteLength(message.id, "utf8") > MAXIMUM_SESSION_ID_BYTES
      || typeof record.sessionId !== "string" || record.sessionId.length < 1
      || Buffer.byteLength(record.sessionId, "utf8") > MAXIMUM_SESSION_ID_BYTES
...
function mergeCanonicalRow(state, row, sourceKeyValue, lineOrdinal, secret, resourceGuard) {
  ...
  for (const block of row.content) {
    ...
    if (typeof block.id !== "string" || block.id.length < 1
        || Buffer.byteLength(block.id, "utf8") > MAXIMUM_SESSION_ID_BYTES) fail("record_invalid");
...
export async function createClaudeTranscriptExportSourcePlan({
...
export function createClaudeTranscriptExportCursor(plan, sourceKeyValue, { secret } = {}) {
  const key = normalizeSecret(secret);
  try {
    assertPlan(plan, key);
...
export function sliceClaudeTranscriptExportSourcePlan(plan, sourceKeyValue, { secret } = {}) {
  const key = normalizeSecret(secret);
  try {
    assertPlan(plan, key);
...
export function sliceClaudeTranscriptExportSourcePlans(plan, { secret } = {}) {
  const key = normalizeSecret(secret);
  try {
    assertPlan(plan, key);
...
export async function verifyClaudeTranscriptExportSource(plan, sourceKeyValue, {
...
export async function scanClaudeTranscriptExportSource(plan, sourceKeyValue, {
...
export function summarizeClaudeTranscriptPlan(plan, { secret } = {}) {
  const key = normalizeSecret(secret);
  try {
    assertPlan(plan, key);
