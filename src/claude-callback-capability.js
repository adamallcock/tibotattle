...
export class ClaudeCallbackCapabilityError extends Error {
  constructor(code) {
    if (!ERROR_CODES.has(code)) throw new TypeError("Unknown Claude callback capability error code");
...
export async function readClaudeCallbackCapability({ backend }) {
...
export async function ensureClaudeCallbackCapability({
  backend,
  generateSecret = () => randomBytes(SECRET_BYTES),
} = {}) {
  const selected = assertBackend(backend);
  if (typeof generateSecret !== "function") fail("invalid_configuration");
  let generated = null;
  let generatedValue = null;
  let readback = null;
  try {
    const existing = await readClaudeCallbackCapability({ backend: selected });
    if (existing !== null) return { status: "existing", secret: existing };
    generatedValue = generateSecret();
    generated = copySecret(generatedValue);
    const outcome = await invoke(selected, "createIfMissing", generated);
    if (!["created", "existing"].includes(outcome)) fail("credential_unavailable");
    readback = await readClaudeCallbackCapability({ backend: selected });
    if (readback === null) fail("credential_unavailable");
    if (outcome === "created" && !timingSafeEqual(readback, generated)) fail("credential_unavailable");
    const result = Buffer.from(readback);
    return { status: outcome, secret: result };
  } finally {
...
export async function rotateClaudeCallbackCapability({
  backend,
  confirm = false,
  generateSecret = () => randomBytes(SECRET_BYTES),
} = {}) {
  ...
  try {
    current = await readClaudeCallbackCapability({ backend: selected });
    if (current === null) fail("credential_missing");
    replacementValue = generateSecret();
    replacement = copySecret(replacementValue);
    const outcome = await invoke(selected, "replaceExact", current, replacement);
...
function confirmationToken(secret, targetBinding) {
  if (typeof targetBinding !== "string" || !/^[a-f0-9]{64}$/.test(targetBinding)) {
    fail("invalid_configuration");
  }
  const digest = createHmac("sha256", secret)
    .update("app-usagemonitor/claude-callback-capability-removal/v1\0")
    .update(targetBinding)
    .digest();
  return digest.subarray(0, 10).toString("hex").toUpperCase();
}

export async function planClaudeCallbackCapabilityRemoval({ backend, targetBinding }) {
  let secret = null;
  try {
    secret = await readClaudeCallbackCapability({ backend });
    if (secret === null) return { status: "missing", confirmationToken: null };
    return { status: "ready", confirmationToken: confirmationToken(secret, targetBinding) };
  } finally {
    secret?.fill(0);
  }
}

export async function removeClaudeCallbackCapability({ backend, targetBinding, providedToken }) {
  if (typeof providedToken !== "string") fail("confirmation_required");
  const selected = assertBackend(backend);
  let secret = null;
  try {
    secret = await readClaudeCallbackCapability({ backend: selected });
    if (secret === null) fail("credential_missing");
    const expected = Buffer.from(confirmationToken(secret, targetBinding));
    const provided = Buffer.from(providedToken);
    const matches = expected.byteLength === provided.byteLength && timingSafeEqual(expected, provided);
    expected.fill(0);
    provided.fill(0);
    if (!matches) fail("confirmation_invalid");
    const outcome = await invoke(selected, "deleteExact", secret);
    if (outcome === "missing") fail("credential_missing");
    if (outcome === "conflict") fail("credential_conflict");
