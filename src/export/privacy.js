...
function scanKeys(value, path = "", findings = []) {
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      scanKeys(item, `${path}/${index}`, findings));
    return findings;
  }
  if (!value || typeof value !== "object") return findings;
  for (const [key, child] of Object.entries(value)) {
    if (FORBIDDEN_KEYS.has(key.toLowerCase())) {
      findings.push({ code: "forbidden_key", path: `${path}/${key}` });
    }
...
function scanStrings(value, path = "", findings = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      scanStrings(item, `${path}/${index}`, findings));
    return findings;
  }
  if (!value || typeof value !== "object") return findings;
  for (const [key, child] of Object.entries(value)) {
...
function recordCountsMatch(bundle) {
  return bundle.recordCounts.usageEvents === bundle.records.usageEvents.length
    && bundle.recordCounts.quotaSnapshots
      === bundle.records.quotaSnapshots.length
    && bundle.recordCounts.activityMarkers
      === bundle.records.activityMarkers.length;
}

function canaryViolations(serialized, forbiddenSourceValues) {
  return forbiddenSourceValues.filter((value) =>
    typeof value === "string"
      && value.length >= 4
      && serialized.includes(value)).length;
}

function providerCompatibilityViolations(bundle) {
  const declared = new Set(
    Array.isArray(bundle.sourceProviders) ? bundle.sourceProviders : [],
  );
  ...
  const adapters = {
    ...
    anthropic_claude_code:
      bundle.compatibility?.providerAdapters?.anthropicClaudeCode,
  };
  let violations = 0;
  for (const provider of observed) {
    if (!declared.has(provider)) violations += 1;
  }
  for (const provider of declared) {
    if (!["implemented", "partial"].includes(adapters[provider]?.status)) {
      violations += 1;
    }
  }
  for (const record of bundle.records?.usageEvents ?? []) {
    if (adapters[record.provider]?.capabilities?.usageEvents !== "implemented") {
      violations += 1;
    }
  }
  for (const record of bundle.records?.quotaSnapshots ?? []) {
    ...
    if (record.provider === "openai_codex") {
      ...
      record.provider === "anthropic_claude_code"
      && record.snapshotSource === "status_line"
    ) {
      capability =
        adapters.anthropic_claude_code?.capabilities?.quotaSnapshots;
    }
...
export function createPrivacySafeBundleVerifier({
  sha256Hex,
  compatibilityTuple,
} = {}) {
  const hash = requireFunction(sha256Hex, "sha256Hex");
  const expectedCompatibility = requireFunction(
    compatibilityTuple,
    "compatibilityTuple",
  );

  return function verifyPrivacySafeBundle(bundle, {
    ...
    const compatibilityMatches =
      stableJson(bundle.compatibility)
      === stableJson(expectedCompatibility());
    const providerViolations = providerCompatibilityViolations(bundle);
    const checks = [
      {
        code: "schema_allowlist",
        status: schemaViolations ? "failed" : "passed",
        violations: schemaViolations,
      },
      {
        code: "compatibility_tuple",
        status: compatibilityMatches ? "passed" : "failed",
        violations: compatibilityMatches ? 0 : 1,
      },
      {
        code: "forbidden_key_scan",
        status: keyFindings.length ? "failed" : "passed",
        violations: keyFindings.length,
      },
      {
        code: "sensitive_string_scan",
        status: stringFindings.length ? "failed" : "passed",
        violations: stringFindings.length,
      },
      {
        code: "record_count_consistency",
        status: recordCountsMatch(bundle) ? "passed" : "failed",
        violations: recordCountsMatch(bundle) ? 0 : 1,
      },
      {
        code: "provider_adapter_compatibility",
        status: providerViolations ? "failed" : "passed",
        violations: providerViolations,
      },
      {
        code: "source_value_canaries",
        status: canaryViolations(serialized, forbiddenSourceValues)
          ? "failed"
          : "passed",
        violations: canaryViolations(
          serialized,
          forbiddenSourceValues,
        ),
      },
    ];
    const verdict = checks.every((check) => check.status === "passed")
      ? "passed"
      : "failed";
    const receipt = {
      ...
      bundleSha256: hash(serialized),
      bundleBytes: UTF8_ENCODER.encode(serialized).byteLength,
      verdict,
    ...
    if (verdict !== "passed") {
      ...
      const sensitiveCodes = [
        ...new Set(stringFindings.map(({ code }) => code)),
      ].sort();
      const sensitiveSuffix = sensitiveCodes.length > 0
        ? `; sensitive=${sensitiveCodes.join("+")}`
        : "";
      throw new Error(
        `Privacy verification failed closed (${failed}${sensitiveSuffix})`,
      );
    }
