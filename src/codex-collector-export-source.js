...
export const CODEX_COLLECTOR_SOURCE_PLAN_VERSION = "codex-collector-export-source-plan-v0.1";
export const CODEX_COLLECTOR_SOURCE_CURSOR_VERSION = "codex-collector-export-cursor-v0.1";
export const CODEX_COLLECTOR_CANDIDATE_VERSION = "codex-collector-quota-candidate-v0.1";
...
export class CodexCollectorExportSourceError extends Error {
  constructor(code) {
...
function assertSafeSourceStats(stats) {
  ...
  if (process.platform !== "win32" && (stats.mode & 0o077) !== 0) fail("source_permissions");
}
...
export async function verifyCodexCollectorExportSourcePlan(plan, {
  resourceGuard = createExportResourceGuard(),
} = {}) {
  validatePlan(plan);
...
export function createCodexCollectorExportCursor(plan) {
  validatePlan(plan);
  return {
 ...
 */
export async function scanCodexCollectorExportSource(plan, {
  cursor = null,
    ...
    if (next.nextByte < plan.prefixBytes) {
      for await (const entry of readBoundedUtf8LineEntries(handle, {
        ...
        for (let windowOrdinal = next.nextWindowOrdinal; windowOrdinal < normalized.windows.length; windowOrdinal += 1) {
          ...
          resourceGuard.observeOutputRecord(Buffer.byteLength(JSON.stringify(candidate), "utf8"));
