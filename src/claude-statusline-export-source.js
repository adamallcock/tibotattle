...
export const CLAUDE_STATUS_LEDGER_SOURCE_PLAN_VERSION = "claude-status-ledger-export-source-plan-v0.1";
export const CLAUDE_STATUS_LEDGER_SOURCE_CURSOR_VERSION = "claude-status-ledger-export-cursor-v0.1";
export const CLAUDE_STATUS_LEDGER_OCCURRENCE_VERSION = "claude-ledger-occurrence-v0.1";
...
export class ClaudeStatusLedgerExportSourceError extends Error {
  constructor(code) {
...
function normalizeSecret(secret) {
  if (!Buffer.isBuffer(secret) && !(secret instanceof Uint8Array)) fail("secret_invalid");
...
function assertSafeRecordStats(stats, expected = null) {
  ...
  if (typeof process.getuid === "function" && stats.uid !== process.getuid()) fail("record_changed");
  if (process.platform !== "win32" && (stats.mode & 0o777) !== 0o600) fail("record_changed");
...
function assertSafePendingStats(stats) {
  ...
  if (typeof process.getuid === "function" && stats.uid !== process.getuid()) fail("ledger_entry");
  if (process.platform !== "win32" && (stats.mode & 0o777) !== 0o600) fail("ledger_entry");
}
...
async function readCanonicalRecord(boundary, record, {
  ...
  try {
    ...
    bytes = Buffer.allocUnsafe(record.size + 1);
    let offset = 0;
    ...
    const verification = Buffer.allocUnsafe(record.size);
...
function inventoryDigest(plan) {
  return sha256(Buffer.from(stableJson(inventoryMaterial(plan)), "utf8"));
}
...
export async function createClaudeStatusLedgerExportSourcePlan({
  stateDirectory,
  startAt,
  endAt,
  secret,
...
export async function verifyClaudeStatusLedgerExportSourcePlan(plan, {
  secret,
  resourceGuard = createExportResourceGuard(),
  failpoint = async () => {},
} = {}) {
...
export function createClaudeStatusLedgerExportCursor(plan, { secret } = {}) {
...
export async function scanClaudeStatusLedgerExportSource(plan, {
  secret,
  cursor = null,
  ...
  while (next.nextRecordIndex < plan.recordCount && records.length < maximumRecords) {
    ...
    resourceGuard.observeOutputRecord(Buffer.byteLength(JSON.stringify(safeRecord), "utf8"));
