...
export function createClaudeTranscriptWorkspaceContext(configuration) {
const {
  bufferByteLength,
...
const CLAUDE_TRANSCRIPT_WORKSPACE_SOURCE_VERSION = "claude-transcript-workspace-source-v0.1";
const DEFAULT_CLAUDE_TRANSCRIPT_RECORDS_PER_BATCH = 500;

const MAXIMUM_BATCH_RECORDS = 1_000;
const SAFE_CODES = new Set(["configuration", "source_integrity"]);

class ClaudeTranscriptWorkspaceSourceError extends Error {
  constructor(code) {
...
async function createClaudeTranscriptWorkspaceSource({
...
function appendClaudeTranscriptWorkspaceSources(supplementalSourcePlan, transcriptPlan, { secret } = {}) {
  let base;
  try {
...
async function populateClaudeTranscriptWorkspaceSources({
  workspace,
  projectsDirectory,
  secret,
  resourceGuard,
