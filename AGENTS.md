...
- Prompts, responses, raw commands, credentials, private paths and filenames,
  raw account IDs, and session content must not enter derived artifacts, fixtures,
  logs, diagnostics, issues, commits, or PRs. Opt-in local Crashpad dumps are
  the sole exception; never project or upload them (`docs/decisions/2026-09-21-opt-in-crash-doctor.md`).
