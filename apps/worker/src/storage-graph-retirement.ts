...
import { STORAGE_GRAPH_CURRENT_FIT_CHECKPOINT_METHOD,STORAGE_GRAPH_V11_FIT_CHECKPOINT_METHOD } from './storage-community-graph';
...
export async function retireStorageGraphPage(target:D1Database,sourceId:string,nowMs=Date.now()):Promise<{
 ...
 const stage=await target.prepare(`SELECT s.source_id,s.owner_digest,s.day,s.dependency_digest,s.source_namespace,s.method,
  ...
  WHERE s.source_id=? AND (s.day<? OR s.owner_digest=? OR h.retired=1 OR (s.method!=? AND EXISTS(
  ...
  .bind(sourceId,oldest,erased?.owner_digest??null,STORAGE_GRAPH_V11_FIT_CHECKPOINT_METHOD,
