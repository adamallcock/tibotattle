...
/** Fixed, isolated storage families; never supplied by telemetry or SQL callers.
 * Both paths share the same source, CAS, bounded framing and cleanup rules. */
export function createCommunityAnalysisWorkStore(namespace: "current" | "model-history" = "current") {
  if (namespace !== "current" && namespace !== "model-history") throw new TypeError("analysis storage namespace invalid");
  const tables = namespace === "current"
    ? { head: "community_analysis_work", parts: "community_analysis_work_parts", stage: "community_analysis_work_stage" }
    : { head: "community_model_history_work", parts: "community_model_history_work_parts", stage: "community_model_history_work_stage" };
  ...
  async function prepareCommunityAnalysisWorkDelta(head: CommunityAnalysisWorkHead, components: unknown): Promise<
  ...
  const WORK_GUARD = `EXISTS (SELECT 1 FROM ${tables.head} h
  ...
  async function beginCommunityAnalysisWork(db: D1Database, identity: CommunityAnalysisWorkIdentity,
    ...
    if (replace) statements.push(db.prepare(`DELETE FROM ${tables.head}
      WHERE participant_id = ? AND run_id = ? AND progress_revision = ? AND ${SOURCE_GUARD}
        AND NOT EXISTS (SELECT 1 FROM ${tables.parts} WHERE participant_id=${tables.head}.participant_id)
        AND NOT EXISTS (SELECT 1 FROM ${tables.stage} WHERE participant_id=${tables.head}.participant_id)`)
  ...
  async function readHead(db: D1Database, identity: Pick<CommunityAnalysisWorkIdentity, "participantId" | "inputRevision">,
    ...
    const row = await db.prepare(`SELECT h.*,p.state AS participant_state,v.revision AS current_input_revision,
      ...
      FROM ${tables.head} h JOIN participants p ON p.id=h.participant_id
  ...
  async function readCommunityAnalysisWork(db: D1Database, identity: CommunityAnalysisWorkIdentity,
  ...
  async function readCommunityAnalysisWorkParts(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function commitCommunityAnalysisWorkPage(db: D1Database, head: CommunityAnalysisWorkHead,
    ...
    const guard = `${WORK_GUARD} AND NOT EXISTS (SELECT 1 FROM ${tables.stage} WHERE participant_id=?)`;
    ...
    for (const part of change.parts) {
      ...
      if ("remove" in part) {
        ...
        statements.push(db.prepare(`INSERT INTO ${tables.parts}
    ...
    for (const part of obsolete.values()) statements.push(db.prepare(`DELETE FROM ${tables.parts}
    ...
    statements.push(db.prepare(`UPDATE ${tables.head} SET phase=?,progress_revision=progress_revision+1,
  ...
  async function readPartRows(db: D1Database, head: CommunityAnalysisWorkHead,
    ...
    if (wanted.length === 0) return (await db.prepare(`SELECT component FROM ${tables.parts} WHERE 0`).all<Record<string, unknown>>()).results;
    return (await db.prepare(`WITH wanted(ordinal,component,part_key,sha256) AS (VALUES ${wanted.map(() => "(?,?,?,?)").join(",")})
      ...
      FROM wanted w JOIN ${tables.parts} p ON p.participant_id=? AND p.run_id=?
  ...
  async function stageGuard(head: CommunityAnalysisWorkHead, stage: CommunityAnalysisWorkStage): Promise<{ sql: string; values: unknown[] }> {
    const current = stage.mode === "discarding"
      ? `EXISTS (SELECT 1 FROM participants p JOIN community_analytical_input_versions v ON v.participant_id=p.id
          JOIN ${tables.head} h ON h.participant_id=p.id WHERE p.id=? AND p.state='active' AND v.revision=?
    ...
    return { sql: `EXISTS (SELECT 1 FROM ${tables.stage} s WHERE s.participant_id=? AND s.run_id=?
  ...
  async function stageUpdate(db: D1Database, head: CommunityAnalysisWorkHead, before: CommunityAnalysisWorkStage,
    ...
    return db.prepare(`UPDATE ${tables.stage} SET stage_revision=?,mode=?,write_offset=?,verified_offset=?,
  ...
  function stageInsert(db: D1Database, head: CommunityAnalysisWorkHead, stage: CommunityAnalysisWorkStage,
    ...
    return db.prepare(`INSERT INTO ${tables.stage}
      ...
      SELECT ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? WHERE ${guard.sql}
        AND NOT EXISTS (SELECT 1 FROM ${tables.stage} WHERE participant_id=?)`)
  ...
  async function beginCommunityAnalysisStage(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function readCommunityAnalysisStage(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function readStageUncharged(db: D1Database, head: CommunityAnalysisWorkHead, inspectRevision?: number): Promise<CommunityAnalysisStageResult> {
    const row = await db.prepare(`SELECT s.*,h.state_sha256 AS head_sha256,p.state AS participant_state,v.revision AS current_revision,
      EXISTS(SELECT 1 FROM telemetry_v11_domain_heads d WHERE d.participant_id=p.id) AS successor_active
      FROM ${tables.stage} s JOIN ${tables.head} h ON h.participant_id=s.participant_id AND h.run_id=s.run_id
  ...
  async function writeCommunityAnalysisStagePage(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function verifyCommunityAnalysisStagePage(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function promoteCommunityAnalysisStage(db: D1Database, head: CommunityAnalysisWorkHead,
    ...
    const updateHead = db.prepare(`UPDATE ${tables.head} SET phase=?,progress_revision=?,control_json=?,manifest_json=?,state_sha256=?
      WHERE participant_id=? AND ${guard.sql}
        AND NOT EXISTS (SELECT 1 FROM json_each(?) j LEFT JOIN ${tables.parts} p
          ON p.participant_id=${tables.head}.participant_id AND p.run_id=${tables.head}.run_id
  ...
  async function beginCommunityAnalysisDiscard(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function beginCommunityAnalysisSupersession(db: D1Database, head: CommunityAnalysisWorkHead,
  ...
  async function beginDiscard(db: D1Database, head: CommunityAnalysisWorkHead,
    ...
    if (previousStage) {
      ...
      statement = db.prepare(`UPDATE ${tables.stage} SET mode='discarding',stage_revision=?,
  ...
  async function readCommunityAnalysisWorkForSupersession(db: D1Database,
  ...
  async function readCommunityAnalysisWorkForDiscard(db: D1Database, participantId: string,
  ...
  async function collectCommunityAnalysisWorkGarbage(db: D1Database, head: CommunityAnalysisWorkHead,
    ...
    const rows = (await db.prepare(`SELECT component,payload_sha256 FROM ${tables.parts}
    ...
    const guard = await stageGuard(head, stage), statements: D1PreparedStatement[] = [];
    if (!rows.length) {
      statements.push(db.prepare(`DELETE FROM ${tables.stage} WHERE participant_id=? AND ${guard.sql}`)
        .bind(head.identity.participantId, ...guard.values));
      if (stage.mode === "discarding") {
        statements.push(db.prepare(`DELETE FROM ${tables.head} WHERE participant_id=? AND run_id=? AND progress_revision=?
          AND changes()=1
          AND NOT EXISTS (SELECT 1 FROM ${tables.parts} WHERE participant_id=? AND run_id=?)
          AND NOT EXISTS (SELECT 1 FROM ${tables.stage} WHERE participant_id=?)
    ...
    for (const row of rows) {
      if (stage.mode === "discarding" || !referenced.has(`${row.component}:${row.payload_sha256}`)) {
        statements.push(db.prepare(`DELETE FROM ${tables.parts} WHERE participant_id=? AND run_id=?
  ...
  return Object.freeze({
    prepareCommunityAnalysisWorkDelta,
    beginCommunityAnalysisWork,
    readCommunityAnalysisWork,
    readCommunityAnalysisWorkParts,
    commitCommunityAnalysisWorkPage,
    beginCommunityAnalysisStage,
    readCommunityAnalysisStage,
    writeCommunityAnalysisStagePage,
    verifyCommunityAnalysisStagePage,
    promoteCommunityAnalysisStage,
    beginCommunityAnalysisDiscard,
...
export type CommunityAnalysisWorkStore = ReturnType<typeof createCommunityAnalysisWorkStore>;
export const communityAnalysisWorkStore = createCommunityAnalysisWorkStore();
export const {
  prepareCommunityAnalysisWorkDelta,
  beginCommunityAnalysisWork,
  readCommunityAnalysisWork,
