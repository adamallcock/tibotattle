...
export async function beginAuthorityRestore(source:D1Database,target:D1Database,contract:AuthorityRestoreContract,pin:string):Promise<void> {
  ...
  for(const object of contract.targetBaseSchema.filter(x=>x.type==='table')){
    ...
    else if(object.name==='storage_owner_move_cursor_contract'){
      // The cursor contract is another role-schema seed. Require its one exact
      // supported version; absence or any additional state still refuses.
      if(canonicalJson((await target.prepare('SELECT * FROM storage_owner_move_cursor_contract LIMIT 2').all()).results)!==canonicalJson([{id:1,version:1}]))throw new Error('AUTHORITY_RESTORE_TARGET_NOT_EMPTY');
    }
