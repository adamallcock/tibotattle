...
async function fixture(){
 ...
 const migrations=[];for(const name of (await readdir(join(root,'migrations'))).filter(name=>/^\d{4}.*\.sql$/.test(name)&&Number(name.slice(0,4))<=59).sort()){const bytes=await readFile(join(root,'migrations',name));await writeFile(join(directory,'canonical',name),bytes,{mode:0o600});migrations.push({name,sha256:sha(bytes),matchesCommit:true});}
