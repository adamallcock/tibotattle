...
async function setup() {
  ...
  await applyD1Migrations(db(), b.TEST_TYPED_V11_ADMISSION_MIGRATIONS.filter(m => m.name.startsWith("0001")));
...
describe("atomic typed v1.1 staged admission", () => {
  ...
  it("rolls back authorization, IDs, headers and records after a late transaction failure", async () => {
    ...
    await db().prepare("CREATE TRIGGER synthetic_late_abort BEFORE INSERT ON typed_v11_record_admissions BEGIN SELECT RAISE(ABORT,'synthetic late failure'); END").run();
  ...
  it("never resets allocated IDs, changes namespaces, or silently admits old v11 history", async () => {
    ...
    await applyD1Migrations(db(), b.TEST_INGESTION_BRIDGE_MIGRATIONS); await applyD1Migrations(db(), b.TEST_TYPED_V11_ADMISSION_MIGRATIONS.filter(m => m.name.startsWith("0001")));
