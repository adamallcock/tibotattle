...
describe("protected telemetry runtime activation", () => {
  ...
  it.each(["primary", "analytics"] as const)("admits a 128-row %s ledger", async (role) => {
    const database = role === "primary" ? db() : analytics();
    const existing = role === "primary" ? EXPECTED_PRIMARY_MIGRATIONS : EXPECTED_ANALYTICS_MIGRATIONS;
    await database.batch(Array.from({ length: 128 - existing.length }, (_, index) => database.prepare(
      "INSERT INTO d1_storage_migrations(name, sha256) VALUES (?, ?)",
    ).bind(`synthetic_boundary_${index}.sql`, "c".repeat(64))));
    await installReconciliationProof();
  ...
  it.each(["primary", "analytics"] as const)("refuses a 129-row %s ledger before activation", async (role) => {
    const database = role === "primary" ? db() : analytics();
    const existing = role === "primary" ? EXPECTED_PRIMARY_MIGRATIONS : EXPECTED_ANALYTICS_MIGRATIONS;
    await database.batch(Array.from({ length: 129 - existing.length }, (_, index) => database.prepare(
      "INSERT INTO d1_storage_migrations(name, sha256) VALUES (?, ?)",
    ).bind(`synthetic_overflow_${index}.sql`, "c".repeat(64))));

    await expect(telemetryRuntimeReconciliationDigests(database)).rejects.toMatchObject({
      code: "TELEMETRY_RUNTIME_ACTIVATION_UNAVAILABLE",
    });
    await expect(activateTelemetryRuntimeAsOwner(
      db(), settings, ACTOR_IDENTITY_KEY, request("usage_v12"), NOW_EPOCH,
  ...
  it("bounds the schema read and refuses its overflow sentinel before activation", async () => {
    let schemaReads = 0;
    const overflowDb = new Proxy(db(), {
      get(target, property, receiver) {
        if (property === "prepare") {
          return (sql: string) => {
            if (sql.includes("SELECT s.type, s.name, s.tbl_name, s.sql")) {
              expect(sql).toMatch(/LIMIT 4097\s*$/u);
              schemaReads += 1;
              return { all: async () => ({ results: Array.from({ length: 4097 }, (_, index) => ({
                type: "table", name: `synthetic_${index}`, tbl_name: `synthetic_${index}`, sql: null,
              })) }) };
            }
    ...
    await expect(activateTelemetryRuntimeAsOwner(
      overflowDb, settings, ACTOR_IDENTITY_KEY, request("usage_v12"), NOW_EPOCH,
    )).rejects.toMatchObject({ code: "TELEMETRY_RUNTIME_ACTIVATION_UNAVAILABLE" });
