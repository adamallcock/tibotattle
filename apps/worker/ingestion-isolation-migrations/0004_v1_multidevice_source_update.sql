-- Full isolated ingestion role, after 0003. An authorized first-revision chunk
-- is a source update even when another device already supplied that day. The
-- deterministic day winner and exact day dependency change, so the affected
-- day is queued and recalculated; public authority and unrelated historical
-- days do not change. Format crossover, correction and replaced same-device
-- occurrences remain hard. The typed device/stream/occurrence unique index
-- rejects same-device stealing atomically.
