-- Shard-local write authority. Install on every qualified ingestion role;
-- never install this schema on the routing catalog as a substitute.
...
CREATE TABLE storage_route_write_checks (
...
CREATE TRIGGER storage_route_write_check_cleanup AFTER INSERT ON storage_route_write_checks BEGIN
  DELETE FROM storage_route_write_checks
  WHERE owner_id = NEW.owner_id
    AND shard_id = NEW.shard_id
    AND route_generation = NEW.route_generation;
END;
...
CREATE TRIGGER storage_owner_fence_transition BEFORE UPDATE ON storage_owner_fences BEGIN
  SELECT (CASE WHEN NEW.owner_id <> OLD.owner_id
    ...
    OR NOT (
      (OLD.state = 'active' AND NEW.state = 'fenced'
        AND NEW.move_id IS NOT NULL AND NEW.copy_digest IS OLD.copy_digest)
