-- 0021: widen the cache-retention band vocabulary to ten.
--
-- The seven-band first cut averaged away the decay it exists to show. The
-- local dashboard's nine buckets separate 1-2m from 2-5m and 5-10m from
-- 10-30m, which is exactly where the curve bends (99.2% -> 97.6%, 95.7% ->
-- 86.4%); a wider bucket reports their mean and hides the bend. The tenth band
-- splits 1-6h, where retention collapses without a six-hour bucket being able
-- to say where.
--
-- SQLite cannot widen a CHECK constraint in place, so the four tables are
-- recreated. Nothing is copied, and that is correct rather than convenient:
-- every existing row carries `method_version = 'cache-retention-v1'` and bands
-- that no longer exist in the vocabulary, so it could never be served under
-- v2. The lane's own retirement sweep reclaims rows whose method version is
-- not the current contract, so these rows were already condemned; this
-- migration only does it in one step instead of gradually. The content is
-- derived and recomputable from the source in full.
--
-- Both versions stay in the CHECK so a reader mid-deploy is never refused by
-- the schema while the worker rolls forward.
...
DROP TABLE IF EXISTS analytics_cache_retention_day_marks;
...
-- The additive index on analytics_v11_reusable_values belongs to 0020 and is
-- not repeated here: that table is not recreated by this migration.
...
CREATE TABLE analytics_cache_retention_day_bands (
  ...
  band TEXT NOT NULL CHECK(band IN ('under_one_minute','one_to_two_minutes','two_to_five_minutes',
    ...
    'one_to_two_hours','two_to_six_hours','six_to_twenty_four_hours','over_twenty_four_hours')),
  source_id TEXT NOT NULL,
  owner_digest TEXT NOT NULL CHECK(length(owner_digest)=64 AND owner_digest NOT GLOB '*[^0-9a-f]*'),
  day TEXT NOT NULL CHECK(length(day)=10 AND day GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'),
  method_version TEXT NOT NULL CHECK(method_version IN ('cache-retention-v1','cache-retention-v2')),
