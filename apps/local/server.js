...
const COLD_REFRESH_V17_PREDECESSOR_PARSERS = Object.freeze([
...
function publishedParserUpgradeNeedsColdRefresh(database, compatibility, schemaVersion) {
  ...
  // worker still validates the complete index. Only reviewed v10 through v16
  // predecessors can receive the v17 rescan window. Their physical schema and
  ...
  if (LOCAL_UNIFIED_INDEX_PARSER_VERSION !== "unified-rollout-typed-v17"
  ...
  `).get(generationId);
  return COLD_REFRESH_V17_PREDECESSOR_PARSERS.includes(generation?.parser_version)
