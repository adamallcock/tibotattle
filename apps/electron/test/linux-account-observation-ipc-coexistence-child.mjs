...
async function run() {
  ...
  const observation = createLinuxAccountObservationBrokerBackendFromEnvironment();
  if (observation === null || process.env.USAGE_MONITOR_LINUX_SECRET_SERVICE_BROKER_FD !== undefined) {
  ...
  try {
    const first = await Promise.all([
      ...
      observation.read(LINUX_ACCOUNT_OBSERVATION_BROKER_CAPABILITY),
    ]);
    ...
    const created = await Promise.all([
      ...
      observation.createIfMissing(LINUX_ACCOUNT_OBSERVATION_BROKER_CAPABILITY, observationSecret),
    ]);
