...
export const WEB_MESSAGES = Object.freeze({
 ...
 "accounting.pricing.coverageReviewed": ["All usage changes in this period have reviewed pricing.", "此期间的所有使用变化都有经审核的价格。", "Todos los cambios de uso de este período tienen precios revisados."],
  ...
  "accounting.cacheContinuity.metricExplanation": ["A conservative estimate for material cache-read drops between adjacent user turns while the effective model, reasoning, routing, and surface stayed unchanged. Elapsed time is evidence rather than an eligibility rule. The headline applies the recorded or selected Codex speed and reviewed Fast multiplier; recorded compactions, contracted contexts, and incomplete boundary coverage are excluded or withheld. It is not a bill or provider-published allowance.", "保守估算：在相邻用户轮次之间，有效模型、推理强度、路由和界面均未改变，但缓存读取量大幅下降。经过时间仅作为证据，而不是资格规则。标题金额会应用已记录或所选的 Codex 速度以及经审核的 Fast 倍数；已记录的上下文压缩、收缩的上下文以及不完整的边界覆盖会被排除或暂缓显示。它不是账单或提供方公布的额度。", "Estimación conservadora de caídas materiales de lectura de caché entre turnos adyacentes sin cambios de modelo, razonamiento, enrutamiento ni superficie. El tiempo es evidencia, no una regla de inclusión. La cifra principal aplica la velocidad de Codex registrada o seleccionada y el multiplicador Fast revisado; las compactaciones, los contextos reducidos y la cobertura incompleta se excluyen o se omiten. No es una factura ni una cuota publicada por el proveedor."],
  ...
  "accounting.cacheContinuity.gapBreakdownUnavailable": ["Time-band breakdown unavailable.", "无法显示时间分段明细。", "El desglose por intervalos de tiempo no está disponible."],
  "accounting.cacheContinuity.gapHeading": ["By time between turns", "按轮次间隔", "Por tiempo entre turnos"],
  "accounting.cacheContinuity.gapTableCaption": ["Possible cache-continuity overhead by time between turns", "按轮次间隔划分的可能缓存连续性开销", "Posible coste adicional de continuidad de caché por tiempo entre turnos"],
  "accounting.cacheContinuity.recentHeading": ["Recent qualifying drops", "近期符合条件的下降", "Caídas recientes que cumplen los requisitos"],
  ...
  "accounting.cacheContinuity.column.gapBand": ["Time between turns", "轮次间隔", "Tiempo entre turnos"],
  "accounting.cacheContinuity.column.dropsComparable": ["Material drops / comparable returns", "大幅下降 / 可比较返回", "Caídas materiales / retornos comparables"],
  ...
  "accounting.cacheContinuity.gapBand.underOneMinute": ["Under 1 minute", "少于 1 分钟", "Menos de 1 minuto"],
  "accounting.cacheContinuity.gapBand.oneToFiveMinutes": ["1–5 minutes", "1–5 分钟", "1–5 minutos"],
  "accounting.cacheContinuity.gapBand.fiveToThirtyMinutes": ["5–30 minutes", "5–30 分钟", "5–30 minutos"],
  "accounting.cacheContinuity.gapBand.thirtyMinutesToOneHour": ["30 minutes–1 hour", "30 分钟–1 小时", "30 minutos–1 hora"],
  "accounting.cacheContinuity.gapBand.oneToSixHours": ["1–6 hours", "1–6 小时", "1–6 horas"],
  "accounting.cacheContinuity.gapBand.sixToTwentyFourHours": ["6–24 hours", "6–24 小时", "6–24 horas"],
  "accounting.cacheContinuity.gapBand.overTwentyFourHours": ["Over 24 hours", "超过 24 小时", "Más de 24 horas"],
  ...
  "installer.trust.eyebrow": ["Download trust", "下载信任", "Confianza de la descarga"],
  "installer.trust.title": ["How can I trust this download?", "如何信任此下载？", "¿Cómo puedo confiar en esta descarga?"],
  "installer.trust.intro": ["TiboTattle is open source. Inspect the source on GitHub, install with Homebrew, or download the DMG directly.", "TiboTattle 是开源软件。你可以在 GitHub 上检查源代码、使用 Homebrew 安装，或直接下载 DMG。", "TiboTattle es de código abierto. Inspecciona el código en GitHub, instala con Homebrew o descarga el DMG directamente."],
  "installer.trust.releaseLabel": ["Release", "发行版", "Versión"],
  "installer.trust.openSource": ["Open source", "开源", "Código abierto"],
  "installer.trust.openSourceDetail": ["Inspect the public source on GitHub or build TiboTattle yourself.", "在 GitHub 上检查公开源代码，或自行构建 TiboTattle。", "Inspecciona el código público en GitHub o compila TiboTattle tú mismo."],
  "installer.trust.developerId": ["Developer ID signed", "Developer ID 签名", "Firmada con Developer ID"],
  "installer.trust.developerIdDetail": ["The distributed app is required to use a hardened-runtime Developer ID signature.", "分发的应用必须使用启用强化运行时的 Developer ID 签名。", "La app distribuida debe usar una firma Developer ID con runtime reforzado."],
  "installer.trust.notarized": ["Apple notarized", "Apple 公证", "Notarizada por Apple"],
  "installer.trust.notarizedDetail": ["Apple notarization, stapling, and Gatekeeper assessment are required before this release is published.", "此版本发布前必须通过 Apple 公证、票据装订和 Gatekeeper 评估。", "Antes de publicar esta versión se requieren la notarización de Apple, el ticket grapado y la evaluación de Gatekeeper."],
  "installer.trust.checksum": ["Published SHA-256", "已发布的 SHA-256", "SHA-256 publicada"],
  "installer.trust.checksumDetail": ["The verified installer metadata publishes this exact checksum:", "经过验证的安装程序元数据发布了这一确切校验和：", "Los metadatos verificados del instalador publican esta suma exacta:"],
  "installer.trust.caveat": ["A checksum proves integrity, not safety. Apple signing and notarization help macOS verify the publisher and artifact, but neither proves source-to-binary provenance or that software is safe.", "校验和证明完整性，而不是安全性。Apple 签名和公证可帮助 macOS 验证发布者和工件，但二者都不能证明源代码到二进制文件的来源关系，也不能证明软件绝对安全。", "Una suma de comprobación demuestra integridad, no seguridad. La firma y la notarización de Apple ayudan a macOS a verificar al editor y el artefacto, pero ninguna demuestra la procedencia del código fuente al binario ni que el software sea seguro."],
  "installer.trust.verifyTitle": ["Verify it yourself", "自行验证", "Verifícalo tú mismo"],
  "installer.trust.verifyIntro": ["With the DMG downloaded, run these commands in Terminal:", "下载 DMG 后，在终端中运行以下命令：", "Con el DMG descargado, ejecuta estos comandos en Terminal:"],
  "installer.trust.verifyChecksum": ["Compare the downloaded DMG with the published checksum.", "将下载的 DMG 与已发布的校验和进行比较。", "Compara el DMG descargado con la suma publicada."],
  "installer.trust.verifyApp": ["Ask macOS Gatekeeper to assess the installed app.", "让 macOS Gatekeeper 评估已安装的应用。", "Pide a Gatekeeper de macOS que evalúe la app instalada."],
  "installer.trust.inspectSignature": ["Inspect the app's signing identity.", "检查应用的签名身份。", "Inspecciona la identidad de firma de la app."],
  "installer.trust.verifyDeep": ["Verify the app signature recursively.", "递归验证应用签名。", "Verifica la firma de la app de forma recursiva."],
  "installer.trust.verifyDmg": ["Assess the DMG signature before opening it.", "在打开 DMG 前评估其签名。", "Evalúa la firma del DMG antes de abrirlo."],
  "installer.trust.verifyNote": ["These checks answer different questions: the checksum checks downloaded bytes, while Apple checks the signature and notarization. None is a cryptographic proof that this binary came from a particular source commit.", "这些检查回答不同的问题：校验和检查下载的字节，而 Apple 检查签名和公证。它们都不是该二进制文件来自特定源代码提交的加密证明。", "Estas comprobaciones responden a preguntas distintas: la suma comprueba los bytes descargados y Apple comprueba la firma y la notarización. Ninguna es una prueba criptográfica de que este binario proceda de un commit concreto."],
  ...
  "community.daily.recomputeNote": ["Each day is an immutable published revision. Late contributions never edit history: they publish a replacement revision, and the revision shown here is always the latest one.", "每一天都是不可变的已发布修订。迟到的贡献绝不会改写历史：它们会发布替代修订，此处显示的始终是最新修订。", "Cada día es una revisión publicada e inmutable. Las contribuciones tardías nunca editan la historia: publican una revisión de reemplazo, y la revisión mostrada aquí es siempre la más reciente."],
...
export const LEGACY_TEXT_CATALOG = Object.freeze({
  ...
  "Download trust": ["下载信任", "Confianza de la descarga"],
  "How can I trust this download?": ["如何信任此下载？", "¿Cómo puedo confiar en esta descarga?"],
  "TiboTattle is open source. Inspect the source on GitHub, install with Homebrew, or download the DMG directly.": ["TiboTattle 是开源软件。你可以在 GitHub 上检查源代码、使用 Homebrew 安装，或直接下载 DMG。", "TiboTattle es de código abierto. Inspecciona el código en GitHub, instala con Homebrew o descarga el DMG directamente."],
  "Release": ["发行版", "Versión"],
  "Open source": ["开源", "Código abierto"],
  "Inspect the public source on GitHub or build TiboTattle yourself.": ["在 GitHub 上检查公开源代码，或自行构建 TiboTattle。", "Inspecciona el código público en GitHub o compila TiboTattle tú mismo."],
  "Developer ID signed": ["Developer ID 签名", "Firmada con Developer ID"],
  "The distributed app is required to use a hardened-runtime Developer ID signature.": ["分发的应用必须使用启用强化运行时的 Developer ID 签名。", "La app distribuida debe usar una firma Developer ID con runtime reforzado."],
  "Apple notarized": ["Apple 公证", "Notarizada por Apple"],
  "Apple notarization, stapling, and Gatekeeper assessment are required before this release is published.": ["此版本发布前必须通过 Apple 公证、票据装订和 Gatekeeper 评估。", "Antes de publicar esta versión se requieren la notarización de Apple, el ticket grapado y la evaluación de Gatekeeper."],
  "Published SHA-256": ["已发布的 SHA-256", "SHA-256 publicada"],
  "The verified installer metadata publishes this exact checksum:": ["经过验证的安装程序元数据发布了这一确切校验和：", "Los metadatos verificados del instalador publican esta suma exacta:"],
  "A checksum proves integrity, not safety. Apple signing and notarization help macOS verify the publisher and artifact, but neither proves source-to-binary provenance or that software is safe.": ["校验和证明完整性，而不是安全性。Apple 签名和公证可帮助 macOS 验证发布者和工件，但二者都不能证明源代码到二进制文件的来源关系，也不能证明软件绝对安全。", "Una suma de comprobación demuestra integridad, no seguridad. La firma y la notarización de Apple ayudan a macOS a verificar al editor y el artefacto, pero ninguna demuestra la procedencia del código fuente al binario ni que el software sea seguro."],
  "Verify it yourself": ["自行验证", "Verifícalo tú mismo"],
  "With the DMG downloaded, run these commands in Terminal:": ["下载 DMG 后，在终端中运行以下命令：", "Con el DMG descargado, ejecuta estos comandos en Terminal:"],
  "Compare the downloaded DMG with the published checksum.": ["将下载的 DMG 与已发布的校验和进行比较。", "Compara el DMG descargado con la suma publicada."],
  "Ask macOS Gatekeeper to assess the installed app.": ["让 macOS Gatekeeper 评估已安装的应用。", "Pide a Gatekeeper de macOS que evalúe la app instalada."],
  "Inspect the app's signing identity.": ["检查应用的签名身份。", "Inspecciona la identidad de firma de la app."],
  "Verify the app signature recursively.": ["递归验证应用签名。", "Verifica la firma de la app de forma recursiva."],
  "Assess the DMG signature before opening it.": ["在打开 DMG 前评估其签名。", "Evalúa la firma del DMG antes de abrirlo."],
  "These checks answer different questions: the checksum checks downloaded bytes, while Apple checks the signature and notarization. None is a cryptographic proof that this binary came from a particular source commit.": ["这些检查回答不同的问题：校验和检查下载的字节，而 Apple 检查签名和公证。它们都不是该二进制文件来自特定源代码提交的加密证明。", "Estas comprobaciones responden a preguntas distintas: la suma comprueba los bytes descargados y Apple comprueba la firma y la notarización. Ninguna es una prueba criptográfica de que este binario proceda de un commit concreto."],
