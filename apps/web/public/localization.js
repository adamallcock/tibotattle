...
export const WEB_MESSAGES = Object.freeze({
  ...
  "dashboard.unavailable.companionInAppCopy": ["The local service stopped. Use Refresh in the TiboTattle toolbar; if it does not recover, quit and reopen the app.", "本地服务已停止。请使用 TiboTattle 工具栏中的“刷新”；如果仍未恢复，请退出并重新打开应用。", "El servicio local se detuvo. Usa Actualizar en la barra de herramientas de TiboTattle; si no se recupera, cierra y vuelve a abrir la app."],
  ...
  "dashboard.pricing.coverageComplete": ["{percent} of recent measured events have a reviewed price.", "近期测量事件中有 {percent} 使用了经审核的价格。", "El {percent} de los eventos medidos recientes tiene un precio revisado."],
  "dashboard.pricing.coverageWithUnpriced": ["{percent} of recent measured events have a reviewed price; {count} remain unpriced.", "近期测量事件中有 {percent} 使用了经审核的价格；仍有 {count} 个未定价。", "El {percent} de los eventos medidos recientes tiene un precio revisado; {count} siguen sin precio."],
  "dashboard.pricing.coverageDenominator": ["Based on {count} measured events in the displayed period.", "基于所显示期间的 {count} 个测量事件。", "Basado en {count} eventos medidos durante el período mostrado."],
  ...
  "dashboard.calibration.noRate": ["TiboTattle needs at least three qualifying matched reset windows with a positive fit before it can estimate this rate and range. API prices remain a measuring stick, not a subscription charge.", "TiboTattle 至少需要三个具有正拟合的合格匹配重置窗口，才能估算此费率和区间。API 价格仍只是衡量尺，而不是订阅费用。", "TiboTattle necesita al menos tres ventanas de restablecimiento coincidentes y válidas con un ajuste positivo antes de estimar esta tasa y su intervalo. Los precios de API siguen siendo una referencia, no un cargo de suscripción."],
  ...
  "contribution.signOutCompleted": ["Signed out. Your server session was revoked and this page forgot the sign-in. Metadata already contributed is unchanged; Hosted privacy controls still let you export or delete it. Sign in again with Google or Apple when you want to contribute.", "已退出登录。你的服务器会话已被撤销，此页面已忘记该登录。已贡献的元数据保持不变；你仍可在托管隐私控制中导出或删除它。想继续贡献时，请使用 Google 或 Apple 再次登录。", "Sesión cerrada. Se revocó tu sesión del servidor y esta página olvidó el inicio de sesión. Los metadatos ya aportados no cambian; los controles de privacidad alojados aún permiten exportarlos o eliminarlos. Inicia sesión de nuevo con Google o Apple cuando quieras contribuir."],
  ...
  "contribution.disconnectConfirmation": ["Disconnect this Mac from contribution?\n\nThis stops future uploads from this Mac, pauses local delivery, and removes this Mac's upload credential. Metadata already contributed is not deleted. You can reconnect later by reviewing a new contribution.", "要断开此 Mac 与贡献服务的连接吗？\n\n这会停止此 Mac 的后续上传、暂停本地传递并移除该 Mac 的上传凭证。已贡献的元数据不会被删除。你稍后可通过审核新的贡献重新连接。", "¿Desconectar este Mac de la contribución?\n\nEsto detiene futuras cargas desde este Mac, pausa la entrega local y elimina la credencial de carga de este Mac. Los metadatos ya aportados no se eliminan. Puedes volver a conectarlo más tarde revisando una nueva contribución."],
  "contribution.disconnectStopping": ["Stopping future uploads from this Mac and removing its local upload credential…", "正在停止此 Mac 的后续上传并移除其本地上传凭证…", "Deteniendo futuras cargas desde este Mac y eliminando su credencial local de carga…"],
  "contribution.disconnectCompleted": ["This Mac is disconnected: future uploads are stopped, local delivery is paused, and its upload credential was removed. Metadata already contributed was not deleted. You can reconnect later by reviewing a new contribution.", "此 Mac 已断开连接：后续上传已停止，本地传递已暂停，其上传凭证已被移除。已贡献的元数据未被删除。你稍后可通过审核新的贡献重新连接。", "Este Mac está desconectado: se detuvieron las futuras cargas, se pausó la entrega local y se eliminó su credencial de carga. Los metadatos ya aportados no se eliminaron. Puedes volver a conectarlo más tarde revisando una nueva contribución."],
  "contribution.disconnectFailed": ["This Mac could not be disconnected, so its local credential was not changed. Retry when the contribution service is reachable.", "无法断开此 Mac，因此其本地凭证未被更改。贡献服务可用时请重试。", "No se pudo desconectar este Mac, por lo que su credencial local no cambió. Vuelve a intentarlo cuando se pueda acceder al servicio de contribución."],
...
export const LEGACY_TEXT_CATALOG = Object.freeze({
  ...
  "The difference between provider-reported quota movement and the movement implied by measured local activity. Quiet periods with no activity and no quota change are neutral, not errors.": ["提供方报告的额度变化与本地测量活动所推算变化之间的差值。没有活动且额度未变化的安静时段是中性状态，不是错误。", "La diferencia entre el movimiento de cuota informado por el proveedor y el implícito en la actividad local medida. Los períodos sin actividad ni cambios de cuota son neutros, no errores."],
  ...
  "31-day cache": ["31 天缓存", "caché de 31 días"],
  "All loaded": ["全部已加载", "Todo lo cargado"],
  "24h": ["24 小时", "24 h"],
  ...
  "Each measured local event uses the public API price that was in effect when it occurred, with the published Fast multiplier when applicable. It is an equivalent for comparison, not an invoice.": ["每个本地测量事件都使用其发生时有效的公开 API 价格，并在适用时采用公开的 Fast 倍数。它是用于比较的等值，而不是账单。", "Cada evento local medido usa el precio público de API vigente cuando ocurrió, con el multiplicador Fast publicado cuando corresponde. Es un equivalente para comparar, no una factura."],
  ...
  "Model usage": ["模型使用情况", "Uso de modelos"],
  ...
  "Measured events": ["测量事件", "Eventos medidos"],
  "Tokens": ["令牌", "Tokens"],
  ...
  "Known models without a reviewed historical price, and genuinely unknown model names, remain visible without an invented cost.": ["缺少经审核历史价格的已知模型以及真正未知的模型名称都会保持可见，且不会编造成本。", "Los modelos conocidos sin un precio histórico revisado y los nombres de modelo realmente desconocidos permanecen visibles sin inventar un coste."],
  "Share one anonymous summary": ["分享一份匿名摘要", "Comparte un resumen anónimo"],
  "Improve community snapshots with one reviewed, content-free result. You see the exact prepared export before anything is sent.": ["用一份经过审核且不含内容的结果改进社区快照。在发送任何内容前，你可以查看确切的准备导出。", "Mejora los resúmenes comunitarios con un resultado revisado y sin contenido. Ves la exportación preparada exacta antes de que se envíe nada."],
  "Improve community estimates with one reviewed, content-free result.": ["用一份经过审核且不含内容的结果改进社区估算。", "Mejora las estimaciones comunitarias con un resultado revisado y sin contenido."],
  "What leaves this Mac — and what never does": ["什么会离开这台 Mac，以及什么永远不会", "Qué sale de este Mac y qué no sale nunca"],
  "Shared: timestamps, token counts, safe model labels, coarse tool and surface categories, and allowance percentages.": ["会分享：时间戳、令牌计数、安全模型标签、粗略的工具和界面类别，以及额度百分比。", "Se comparten: marcas de tiempo, recuentos de tokens, etiquetas de modelo seguras, categorías generales de herramientas y superficies, y porcentajes de límite."],
  "Never shared: prompts, responses, reasoning, files, paths, URLs, commands, account names, emails, identifiers, or credentials.": ["绝不分享：提示词、回复、推理、文件、路径、URL、命令、帐户名称、电子邮件、标识符或凭据。", "Nunca se comparten: indicaciones, respuestas, razonamiento, archivos, rutas, URL, comandos, nombres de cuenta, correos, identificadores ni credenciales."],
  ...
  "Hosted participation uses Google or Apple; the service keeps only an irreversible sign-in hash, never your email or name. Local-only use needs no account.": ["托管参与使用 Google 或 Apple；服务只保留不可逆的登录哈希，绝不保留你的电子邮件或姓名。仅本地使用不需要帐户。", "La participación alojada usa Google o Apple; el servicio solo conserva un hash irreversible del inicio de sesión, nunca tu correo ni tu nombre. El uso exclusivamente local no requiere cuenta."],
  ...
  "Signing out revokes this browser's service session and forgets this sign-in. Hosted privacy controls remain available separately.": ["退出登录会撤销此浏览器的服务会话并忘记这次登录。托管隐私控制仍可单独使用。", "Cerrar sesión revoca la sesión de servicio de este navegador y olvida este inicio de sesión. Los controles de privacidad alojados siguen disponibles por separado."],
  "This browser already has a service session. Signing out revokes it; hosted privacy controls remain available separately.": ["此浏览器已有服务会话。退出登录会撤销该会话；托管隐私控制仍可单独使用。", "Este navegador ya tiene una sesión de servicio. Cerrar sesión la revoca; los controles de privacidad alojados siguen disponibles por separado."],
  "Stop this Mac's future uploads without deleting metadata already contributed. Reconnecting later requires a fresh reviewed contribution on this Mac.": ["停止此 Mac 的后续上传，而不删除已贡献的元数据。稍后重新连接需要在此 Mac 上审核一份新的贡献。", "Detén las futuras cargas de este Mac sin eliminar los metadatos ya aportados. Volver a conectarlo más tarde requiere una nueva contribución revisada en este Mac."],
  "Disconnect this Mac": ["断开此 Mac 的连接", "Desconectar este Mac"],
  ...
  "06 · Community contribution": ["06 · 社区贡献", "06 · Contribución comunitaria"],
  "Review first. Contribute by choice.": ["先审阅，再自主贡献。", "Revisa primero. Contribuye por elección."],
  "Review one content-free result before an optional upload. Local reporting stays on this Mac.": ["在可选上传前审阅一份不含内容的结果。本地报告保留在此 Mac 上。", "Revisa un resultado sin contenido antes de una carga opcional. Los informes locales permanecen en este Mac."],
