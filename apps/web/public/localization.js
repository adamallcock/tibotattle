...
export const WEB_MESSAGES = Object.freeze({
 ...
 "accounting.pricing.coverageReviewed": ["All usage changes in this period have reviewed pricing.", "此期间的所有使用变化都有经审核的价格。", "Todos los cambios de uso de este período tienen precios revisados."],
  ...
  "accounting.fastMode.assumedRatio": ["Includes {amount} of Standard-rate cost weighted at an assumed 2x because the model has no published Priority rate.", "其中 {amount} 的 Standard 费率成本因该模型没有已公布的 Priority 价格而按假定的 2 倍加权。", "Incluye {amount} de coste a tarifa Standard ponderado por un 2x asumido porque el modelo no tiene un precio Priority publicado."],
...
export const LEGACY_TEXT_CATALOG = Object.freeze({
  ...
  "This Mac still has an upload credential from an older build. TiboTattle moves it into app-owned keychain storage the next time it is needed. If macOS asks then, allow TiboTattle to continue; declining leaves the old credential untouched and pauses uploads.": ["这台 Mac 仍有一个来自旧版本的上传凭据。下次需要该凭据时，TiboTattle 会将它迁移到应用自有的钥匙串存储中。如果 macOS 届时询问，请允许 TiboTattle 继续；拒绝会保留旧凭据并暂停上传。", "Este Mac aún tiene una credencial de carga de una versión anterior. TiboTattle la trasladará al almacenamiento del llavero propio de la app la próxima vez que se necesite. Si macOS pregunta, permite que TiboTattle continúe; si lo rechazas, la credencial anterior se conserva y las cargas se pausan."],
