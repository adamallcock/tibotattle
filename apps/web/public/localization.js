...
export const WEB_MESSAGES = Object.freeze({
 ...
 "accounting.pricing.coverageReviewed": ["All usage changes in this period have reviewed pricing.", "此期间的所有使用变化都有经审核的价格。", "Todos los cambios de uso de este período tienen precios revisados."],
  ...
  "community.metric.allTokens": ["All tokens", "全部 token", "Todos los tokens"],
  ...
  "community.daily.apiEquivalentSpend": ["Total API-equivalent spend", "API 等值总支出", "Gasto total equivalente de API"],
  "community.daily.apiEquivalentSpendDetail.complete": ["Across {days} shared days in USD; not an actual bill", "跨 {days} 个共享日期，以美元计；非实际账单", "En {days} días compartidos, en USD; no es una factura real"],
  "community.daily.apiEquivalentSpendDetail.partial": ["Priced portion across {pricedDays} of {days} shared days in USD; not an actual bill", "{days} 个共享日期中 {pricedDays} 天的已定价部分，以美元计；非实际账单", "Parte con precio de {pricedDays} de {days} días compartidos, en USD; no es una factura real"],
  "community.daily.apiEquivalentSpendDetail.unavailable": ["API-equivalent spend unavailable; not an actual bill", "API 等值支出不可用；非实际账单", "Gasto equivalente de API no disponible; no es una factura real"],
  ...
  "community.daily.allTokensCounted": ["All tokens counted", "已计入的全部 token", "Todos los tokens contabilizados"],
  "community.daily.allTokensCountedDetail": ["Input and output across {days} shared days", "跨 {days} 个共享日期的输入与输出", "Entrada y salida en {days} días compartidos"],
  ...
  "community.daily.chartDescription": ["Usage events per published day as bars, with all input and output tokens as a line on a second axis. Unpublished days and unavailable token totals appear as gaps.", "每个已发布日期的使用事件以柱形显示，全部输入和输出 token 以第二坐标轴上的折线显示。未发布的日期和不可用的 token 总量显示为空缺。", "Los eventos de uso por día publicado se muestran como barras, y todos los tokens de entrada y salida como una línea en un segundo eje. Los días sin publicar y los totales de tokens no disponibles aparecen como huecos."],
  ...
  "community.allowance.heroCopy": ["Compare the aggregate, plan groups, or individual models on the same Pro 20x-equivalent weekly allowance, valued at API prices.", "以相同的 Pro 20x 等值周额度比较汇总、方案分组或单个模型，按 API 价格计值。", "Compara el agregado, los grupos de planes o los modelos con la misma asignación semanal equivalente a Pro 20x, valorada a precios de API."],
  ...
  "community.allowance.viewLabel": ["Allowance graph view", "额度图表视图", "Vista del gráfico de asignación"],
  ...
  "community.allowance.smallSampleDisclosure": ["Estimates and sample counts may be based on a single account. Small samples are uncertain and can reveal that contributor’s estimated capacity, even though no account identifiers are published.", "估计值和样本数可能仅基于一个账户。小样本存在不确定性，即使不公开账户标识，也可能透露该贡献者的估计额度。", "Las estimaciones y los recuentos pueden basarse en una sola cuenta. Las muestras pequeñas son inciertas y pueden revelar la capacidad estimada de ese colaborador, aunque no se publiquen identificadores de cuenta."],
  ...
  "community.allowance.planMethod": ["Plan groups are normalized to the same Pro 20× weekly allowance: Pro 20× ×1, Pro 5× ×4, Plus ×20. These are comparisons, not each plan’s actual dollar allowance. Small samples are uncertain; missing days remain gaps.", "方案分组均换算为相同的 Pro 20× 周额度：Pro 20× 乘以 1，Pro 5× 乘以 4，Plus 乘以 20。这些是比较值，并非各方案实际的美元额度。小样本存在不确定性；缺失日期保持为空缺。", "Los grupos se normalizan a la misma asignación semanal Pro 20×: Pro 20× ×1, Pro 5× ×4 y Plus ×20. Son comparaciones, no la asignación real en dólares de cada plan. Las muestras pequeñas son inciertas; los días sin datos quedan vacíos."],
  "community.allowance.modelMethod": ["Estimated API-equivalent value if a full Pro 20× weekly allowance were used on one model. Only identified model fits appear; sample counts are supporting accounts, not reset fits. Historical estimates are reconstructed only from evidence available through each day. Missing or unstable estimates remain gaps, and model availability is not implied.", "假设将完整的 Pro 20× 周额度用于单一模型时，估计的 API 等值金额。仅展示可识别的模型拟合；样本数指支持该估计的账户，并非重置拟合数。历史估计仅根据截至当日的证据重建。缺失或不稳定的估计保持为空缺，不表示模型当前可用。", "Valor equivalente de API estimado si toda una asignación semanal Pro 20× se usara en un solo modelo. Solo aparecen ajustes de modelos identificados; las muestras son cuentas, no ajustes de reinicio. El historial se reconstruye solo con evidencia hasta cada fecha. Las estimaciones ausentes o inestables dejan huecos y no implican disponibilidad del modelo."],
