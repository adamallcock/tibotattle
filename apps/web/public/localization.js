...
export const LEGACY_TEXT_CATALOG = Object.freeze({
  ...
  "Each contributor is capped, and weekly figures are delayed, rounded, and shown only when enough independent people support them.": ["每位贡献者都有上限；每周数值会延迟、取整，并且仅在有足够独立参与者支持时显示。", "Cada colaborador tiene un límite y las cifras semanales se retrasan, se redondean y solo se muestran cuando las respaldan suficientes personas independientes."],
  ...
  "Community values use a fixed delay, independent per-cell support, participant-level clipping, and coarse rounding. A sealed revision is never rewritten; privacy-affecting deletion withdraws it and schedules a replacement revision without the deleted source.": ["社区数值采用固定延迟、每个单元格独立支持、参与者级别截断和粗粒度取整。已封存的修订绝不会被重写；影响隐私的删除会撤回该修订，并安排不含已删除来源的替代修订。", "Los valores comunitarios usan un retraso fijo, soporte independiente por celda, recorte por participante y redondeo grueso. Una revisión sellada nunca se reescribe; una eliminación que afecta a la privacidad la retira y programa una revisión de sustitución sin la fuente eliminada."],
