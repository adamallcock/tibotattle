...
export const WEB_MESSAGES = Object.freeze({
 ...
 "accounting.pricing.coverageReviewed": ["All usage changes in this period have reviewed pricing.", "此期间的所有使用变化都有经审核的价格。", "Todos los cambios de uso de este período tienen precios revisados."],
  ...
  "community.allowance.heroCopy": ["Estimated weekly API-price value: Pro 20× equivalent overall and by model; each plan's own week in By plan.", "估计每周 API 价格价值：汇总和按模型使用 Pro 20× 等值；按方案显示各方案自身的一周额度。", "Valor semanal estimado a precios de API: equivalente Pro 20× en conjunto y por modelo; la semana propia de cada plan en Por plan."],
  ...
  "community.allowance.planChartDescription": ["A separate estimate of each plan’s own week at API prices, on its own scale. Shading shows the middle 80% of qualified reset fits when available. Missing days remain gaps. Arrow keys inspect individual points.", "按 API 价格分别估计每个方案自身的一周额度，并使用各自刻度。有足够数据时，阴影显示合格重置拟合的中间 80% 范围。缺失日期保持为空缺。使用方向键查看各点。", "Una estimación de la semana propia de cada plan a precios de API, con su propia escala. El sombreado muestra el 80% central de los ajustes válidos cuando está disponible. Los días ausentes quedan vacíos. Las flechas recorren los puntos."],
  ...
  "community.allowance.planMethod": ["Each plan is charted at its own week at API prices, on its own scale. Card values repeat that figure, with the Pro 20× equivalent beneath it. Pro 50× uses a provisional ×0.4 conversion to the reference. Shading: middle 80% of qualifying reset fits. Missing days stay gaps.", "每个方案均按其自身每周 API 价格绘制，各自使用独立刻度。卡片数值与之相同，下方为 Pro 20× 等效值。Pro 50× 暂按 ×0.4 换算为参考方案。阴影：合格重置拟合的中间 80%。缺失日期保持为空隙。", "Cada plan se representa con su propia semana a precios de API y su propia escala. Las tarjetas repiten esa cifra, con el equivalente Pro 20× debajo. Pro 50× usa una conversión provisional de ×0,4 a la referencia. Sombreado: 80% central de los ajustes de reinicio válidos. Los días ausentes quedan como huecos."],
  "community.allowance.planMethodLegacy": ["Each plan is charted at its own week at API prices, on its own scale. Card values repeat that figure, with the Pro 20× equivalent beneath it. Shading: middle 80% of qualifying reset fits. Missing days stay gaps.", "每个方案均按其自身每周 API 价格绘制，各自使用独立刻度。卡片数值与之相同，下方为 Pro 20× 等效值。阴影：合格重置拟合的中间 80%。缺失日期保持为空隙。", "Cada plan se representa con su propia semana a precios de API y su propia escala. Las tarjetas repiten esa cifra, con el equivalente Pro 20× debajo. Sombreado: 80% central de los ajustes de reinicio válidos. Los días ausentes quedan como huecos."],
  ...
  "community.allowance.methodNote": ["Each point combines qualifying reset fits from the trailing 30 days after normalizing supported personal plans to a Pro 20× equivalent (Pro ×1, Pro 5× ×4, provisional Pro 50× ×0.4, Plus ×20). The line is the median, the range is the middle 80%, and every fit passes the shared calibration gates including the 25-point observed-span floor. Unsupported plans are excluded; late data republishes the day.", "每个点都汇总过去 30 天内的合格重置拟合，并将支持的个人方案换算为 Pro 20× 等值（Pro ×1、Pro 5× ×4、暂定 Pro 50× ×0.4、Plus ×20）。折线为中位数，区间为中间 80%；每个拟合均通过共享校准门槛，包括 25 个百分点的观测跨度下限。不支持的方案会被排除；迟到的数据会重新发布对应日期。", "Cada punto combina los ajustes de restablecimiento válidos de los últimos 30 días tras normalizar los planes personales compatibles a un equivalente de Pro 20× (Pro ×1, Pro 5× ×4, Pro 50× provisional ×0,4 y Plus ×20). La línea es la mediana, el rango es el 80 % central y cada ajuste supera las puertas de calibración compartidas, incluido el mínimo de 25 puntos de amplitud observada. Los planes no compatibles se excluyen; los datos tardíos vuelven a publicar el día."],
