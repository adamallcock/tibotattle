...
export const WEB_MESSAGES = Object.freeze({
 ...
 "accounting.pricing.coverageReviewed": ["All usage changes in this period have reviewed pricing.", "此期间的所有使用变化都有经审核的价格。", "Todos los cambios de uso de este período tienen precios revisados."],
  ...
  "accounting.cacheSwitch.noteIncompleteOrdering": ["Whole-period total unavailable. Sessions without exact local event order: {ordering}.", "整个期间的总额不可用。缺少精确本地事件顺序的会话：{ordering} 个。", "Total del período completo no disponible. Sesiones sin orden local exacto de eventos: {ordering}."],
  ...
  "accounting.cacheContinuity.noteIncompleteOrdering": ["Whole-period total unavailable. Sessions without exact local event order: {ordering}.", "整个期间的总额不可用。缺少精确本地事件顺序的会话：{ordering} 个。", "Total del período completo no disponible. Sesiones sin orden local exacto de eventos: {ordering}."],
  ...
  "accounting.cacheContinuity.outcome.readoutSubtotal": ["Covered/priced subtotal at Standard rates: {amount}. Priced drops: {priced}; unpriced excluded: {unpriced}. Whole-period total unavailable.", "按 Standard 费率计算的已覆盖且已定价小计：{amount}。已定价下降：{priced} 次；排除的未定价下降：{unpriced} 次。整个期间的总额不可用。", "Subtotal con cobertura y tarifas Standard: {amount}. Caídas con precio: {priced}; sin precio excluidas: {unpriced}. Total del período completo no disponible."],
