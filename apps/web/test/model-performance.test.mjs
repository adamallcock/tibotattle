...
test('background history scan reports honest bounded progress while keeping charts visible', async () => {
  ...
  assert.equal(dom.root.all().filter(node => node.dataset.evidence === 'freshness').length, 1, 'progress retains the timestamp of the displayed results');
  assert.deepEqual(dom.root.all().filter(node => node.className === 'performance-unit').map(node => node.dataset.state), ['partial', 'partial']);
  assert.equal(dom.root.all().some(node => node.dataset.evidence === 'speed-coverage'), false, 'counts are not repeated above the charts');
  controller.destroy();
});
