...
export async function writeAttributionFixture(
  ...
  const generation = beginUnifiedIndexGeneration(database, {
    contractVersion: "usage-event-v0.2", receivedAtMs: observedAtMs,
  ...
  for (const [index, planType] of plans.entries()) {
    ...
    const eventObservedAtMs = observedAtMs + index * 1_000;
    const window = { observedAtMs: eventObservedAtMs, limitId: "codex", slot: "secondary", planType, usedPercent: 10 + index,
      resetsAtMs: observedAtMs + 604_800_000, durationMins: 10_080 };
    ...
    writer.writeUsageEvent({ ...source, eventKey, observedAtMs: eventObservedAtMs, sourceOffset: index + 1, generationId: generation.generationId,
  ...
  writer.writeToolClassFact({ ...source, eventKey: Buffer.alloc(32, 71), sourceOffset: 1,
    generationId: generation.generationId, observedAtMs,
    toolOrdinal: 0, toolClass: "localShell", sourceKind: "response_item" });
  writer.writeSourceCursor({ ...source, scannedBytes: 4096, sizeBytes: 4096, mtimeMs: observedAtMs,
  ...
  writer.writeGenerationSource({ ...source, generationId: generation.generationId, surfaceId, status: "complete",
    discoveredSizeBytes: 4096, scannedBytes: 4096, mtimeMs: observedAtMs, diagnosticsComplete: true });
