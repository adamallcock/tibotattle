...
test("executed scanner version is the version embedded in the bundle", async () => {
  ...
  try {
    assert.equal(result.bundle.compatibility.providerAdapters.openaiCodex.sourceFormats.rollout.parserVersion, "codex-log-scan-v8");
    assert.equal(result.receipt.compatibility.providerAdapters.openaiCodex.sourceFormats.rollout.parserVersion, "codex-log-scan-v8");
  } finally {
