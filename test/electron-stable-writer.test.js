...
async function fixture(run, { predecessors = false } = {}) {
  ...
  try {
    for (const target of STABLE_TARGETS) {
      ...
      const names = target.startsWith('darwin-') ? ['zip','dmg'].flatMap(ext => [`TiboTattle-${RELEASE_VERSION}-mac-${spec.architecture}.${ext}`, `TiboTattle-${RELEASE_VERSION}-mac-${spec.architecture}.${ext}.blockmap`]) : [target === 'win32-x64' ? `TiboTattle-${RELEASE_VERSION}-Windows-x64.exe` : `TiboTattle-${RELEASE_VERSION}-linux-x64.AppImage`];
