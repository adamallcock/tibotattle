...
const CERTIFICATE_SUBJECT_SHA256 = "2".repeat(64);
const PREPARATION_HANDOFF_SHA = "6".repeat(64);
...
test("joins canonical evidence into a frozen, content-free not-ready receipt", async () => {
  ...
  try {
    ...
    assert.equal(
      receipt.authenticode.receipt.files.every(
        ({ authenticode }) => authenticode.subjectSha256 === CERTIFICATE_SUBJECT_SHA256,
      ),
      true,
    );
...
test("rejects independently valid but spliced ledger and PE inventory subjects", async () => {
  ...
  try {
    ...
    for (const role of ["installer", "electron-dll", "main-executable"]) {
      const subjectSplicedRows = value.inventory.files.map((row) => row.role === role
        ? {
          ...row,
          authenticode: {
            ...row.authenticode,
        ...
        : row);
      const subjectSplicedInventory = {
        ...value.inventory,
        files: subjectSplicedRows,
        inventory: {
          ...inventoryDigest(subjectSplicedRows),
          roles: { ...value.inventory.inventory.roles },
        },
      };
      const subjectSplicedInventoryBytes = Buffer.from(
        serializeWindowsProductionAuthenticodeInventoryReceipt(subjectSplicedInventory),
        "utf8",
      );
      const subjectSplicedInstaller = {
        ...value.installer,
        authority: {
          ...value.installer.authority,
          inventorySha256: sha256(subjectSplicedInventoryBytes),
        },
      };
      const subjectSplicedInstallerBytes = Buffer.from(
        serializeWindowsProductionInstallerReceipt(subjectSplicedInstaller),
        "utf8",
      );
      await assert.rejects(
        buildWindowsProductionFinalizerReceiptForTest(
          options(value, {
            authenticodeInventoryBytes: subjectSplicedInventoryBytes,
            installerBytes: subjectSplicedInstallerBytes,
          }),
        ),
        expectCode(STATUS.mismatch),
        `same-SimpleName subject splice for ${role}`,
      );
    }
  } finally {
    await rm(value.parent, { recursive: true, force: true });
