import { createHash } from "node:crypto";
import { readFileSync } from "node:fs";
import assert from "node:assert/strict";
import test from "node:test";

import {
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_ACTIONS,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_CONFIRMATION_INPUT,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_CONFIRMATION_VALUE,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_DISPATCH_CONTEXT,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_METADATA_BINDING,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_MAXIMUM_BASE64_BYTES,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_MAXIMUM_BYTES,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_COUNT,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_MAXIMUM_BYTES,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_SCRIPT,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_RECEIPTS,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_STAGE_ORDER,
  WINDOWS_PRODUCTION_SIGNED_FINALIZER_WORKFLOW_POLICY,
} from "../config/windows-production-signed-finalizer-workflow-contract.js";

const WORKFLOW_PATH = ".github/workflows/windows-production-finalizer-signed.yml";
const workflow = readFileSync(WORKFLOW_PATH, "utf8");
const PREPARATION_TRANSPORT_PATH = "scripts/windows-preparation-manifest-transport.ps1";
const preparationTransport = readFileSync(PREPARATION_TRANSPORT_PATH, "utf8");
const ATTEMPT_AZURE_CONFIG_DIR = "${{ runner.temp }}/tibotattle-windows-production-finalizer-signed-${{ github.run_id }}-${{ github.run_attempt }}/azure-config";

function mustInclude(fragment, message = fragment) {
  assert.equal(workflow.includes(fragment), true, `workflow must include ${message}`);
}

function mustNotInclude(fragment, message = fragment) {
  assert.equal(workflow.includes(fragment), false, `workflow must not include ${message}`);
}

function jobSection(name) {
  const marker = `  ${name}:\n`;
  const start = workflow.indexOf(marker);
  assert.ok(start >= 0, `workflow must include job ${name}`);
  const remainder = workflow.slice(start + marker.length);
  const nextJob = remainder.search(/\n  [A-Za-z0-9_-]+:\n/u);
  return workflow.slice(start, nextJob < 0 ? workflow.length : start + marker.length + nextJob);
}

function stepOffset(name, section = workflow) {
  const marker = `      - name: ${name}\n`;
  const offset = section.indexOf(marker);
  assert.ok(offset >= 0, `workflow must include step ${name}`);
  return offset;
}

function stepSection(name, section = workflow) {
  const marker = `      - name: ${name}\n`;
  const start = stepOffset(name, section);
  const end = section.indexOf("\n      - name:", start + marker.length);
  return section.slice(start, end < 0 ? section.length : end);
}

test("signed finalizer is manual, protected, non-cancellable, and has exact permissions", () => {
  mustInclude("name: Windows production finalizer signed candidate");
  mustInclude("on:\n  workflow_dispatch:");
  for (const trigger of ["push:", "pull_request:", "schedule:", "workflow_call:", "workflow_run:"]) {
    mustNotInclude(`\n${trigger}`, `${trigger} trigger`);
  }
  mustInclude("permissions:\n  contents: read\n  actions: read");
  mustNotInclude("permissions:\n  contents: read\n  actions: read\n  id-token: write", "workflow-wide OIDC");
  mustInclude("runs-on: windows-2025");
  mustInclude("environment: windows-production-signing");
  mustInclude("prepare:\n");
  mustInclude("sign:\n    name: Windows signed candidate (unpublished)");
  mustInclude("needs: prepare");
  assert.match(jobSection("prepare"), /permissions:\n      contents: read\n      actions: read/u);
  assert.doesNotMatch(jobSection("prepare"), /id-token\s*:/u);
  assert.doesNotMatch(jobSection("prepare"), /environment:\s*windows-production-signing/u);
  assert.match(jobSection("sign"), /permissions:\n      contents: read\n      actions: read\n      id-token: write/u);
  mustInclude("group: windows-production-finalizer-signed-${{ inputs.source_revision }}");
  mustInclude("cancel-in-progress: false");
  mustInclude(`      ${WINDOWS_PRODUCTION_SIGNED_FINALIZER_CONFIRMATION_INPUT}:`);
  mustInclude(WINDOWS_PRODUCTION_SIGNED_FINALIZER_CONFIRMATION_VALUE);
  assert.deepEqual(WINDOWS_PRODUCTION_SIGNED_FINALIZER_WORKFLOW_POLICY.workflow.permissions, {
    contents: "read",
    actions: "read",
  });
});

test("source provenance is proven before checkout and no target code runs before it", () => {
  const dispatch = stepOffset("Validate exact dispatch inputs and explicit confirmation");
  const provenance = stepOffset("Fetch and statically validate exact source provenance before checkout");
  const checkout = stepOffset("Check out the exact source revision");
  assert.ok(dispatch < provenance && provenance < checkout);
  const preCheckout = workflow.slice(dispatch, checkout);
  for (const pattern of [
    /\bnode(?:\.exe)?\b/iu,
    /\b(?:npm|pnpm)\b/iu,
    /(?:^|[\\/])scripts[\\/]/iu,
    /package\.json/iu,
    /(?:^|[\\/])config[\\/]/iu,
    /\b(?:Import|Get)-Module\b/iu,
  ]) {
    assert.doesNotMatch(preCheckout, pattern, `target code must not run before checkout: ${pattern}`);
  }
  const provenanceSection = stepSection("Fetch and statically validate exact source provenance before checkout");
  assert.match(provenanceSection, /gh api --method GET --header 'Accept: application\/vnd\.github\+json'/u);
  assert.match(provenanceSection, /ConvertFrom-Json/u);
  assert.match(provenanceSection, /ReparsePoint/u);
  assert.match(provenanceSection, /sourceRun\.head_sha/u);
  assert.match(provenanceSection, /sourceRun\.conclusion -cne 'success'/u);
  assert.match(provenanceSection, /paginationPresent\.Count -ne 0/u);
});

test("the finalizer dispatch context is validated before checkout and metadata is bound to it", () => {
  const dispatch = stepSection("Validate exact dispatch inputs and explicit confirmation");
  const dispatchOffset = stepOffset("Validate exact dispatch inputs and explicit confirmation");
  const checkoutOffset = stepOffset("Check out the exact source revision");
  assert.ok(dispatchOffset < checkoutOffset);
  assert.ok(dispatch.includes(
    `FINALIZER_DISPATCH_CONTEXT_REF: ${WINDOWS_PRODUCTION_SIGNED_FINALIZER_DISPATCH_CONTEXT.refExpression}`,
  ));
  assert.ok(dispatch.includes(
    `FINALIZER_DISPATCH_CONTEXT_SHA: ${WINDOWS_PRODUCTION_SIGNED_FINALIZER_DISPATCH_CONTEXT.shaExpression}`,
  ));
  assert.match(dispatch, /FINALIZER_DISPATCH_CONTEXT_REF -cne 'refs\/heads\/main'/u);
  assert.match(dispatch, /FINALIZER_DISPATCH_CONTEXT_SHA -cne \$env:SOURCE_REVISION/u);
  assert.match(dispatch, /TIBOTATTLE_VALIDATED_FINALIZER_REF=\$\(\$env:FINALIZER_DISPATCH_CONTEXT_REF\)/u);
  assert.match(dispatch, /TIBOTATTLE_VALIDATED_FINALIZER_SHA=\$\(\$env:FINALIZER_DISPATCH_CONTEXT_SHA\)/u);

  const seed = stepSection("Seed canonical fixed evidence inputs without clobbering");
  assert.match(seed, /FINALIZER_DISPATCH_CONTEXT_REF: \$\{\{ github\.ref \}\}/u);
  assert.match(seed, /FINALIZER_DISPATCH_CONTEXT_SHA: \$\{\{ github\.sha \}\}/u);
  assert.match(seed, /validatedRef !== dispatchRef/u);
  assert.match(seed, /validatedSha !== dispatchSha/u);
  assert.match(seed, /headSha: validatedSha/u);
  assert.match(seed, /ref: validatedRef/u);
  assert.doesNotMatch(seed, /headSha: revision/u);
  assert.doesNotMatch(seed, /ref: ["']refs\/heads\/main["']/u);
  assert.equal(
    WINDOWS_PRODUCTION_SIGNED_FINALIZER_METADATA_BINDING.ref,
    "TIBOTATTLE_VALIDATED_FINALIZER_REF",
  );
  assert.equal(
    WINDOWS_PRODUCTION_SIGNED_FINALIZER_METADATA_BINDING.headSha,
    "TIBOTATTLE_VALIDATED_FINALIZER_SHA",
  );
});

test("all external actions are pinned to full commit SHAs", () => {
  for (const reference of Object.values(WINDOWS_PRODUCTION_SIGNED_FINALIZER_ACTIONS)) mustInclude(reference);
  for (const line of workflow.split("\n").filter((value) => value.trimStart().startsWith("uses:"))) {
    assert.match(line, /uses: [^@\s]+@[0-9a-f]{40}(?:\s|$)/u);
  }
  assert.equal(
    (workflow.match(/^\s+uses: actions\/download-artifact@[0-9a-f]{40}/gmu) ?? []).length,
    4,
  );
});

test("toolchain, credential gates, Azure login, and signing order are explicit", () => {
  for (const fragment of [
    "node-version: 26.2.0",
    "corepack@0.34.0",
    "pnpm@11.9.0",
    "pnpm install --frozen-lockfile",
    "Install-Module -Name TrustedSigning -RequiredVersion 0.5.0",
    "verify-windows-trusted-signing-preflight.mjs",
    "verify-windows-production-certificate-subject-preflight.mjs",
    "AZURE_CODE_SIGNING_SUBJECT_SHA256",
    "AZURE_CONFIG_DIR",
    "TIBOTATTLE_ELECTRON_SIGNING_MODE: azure-trusted-signing",
    "--config apps/electron/electron-builder.release.config.cjs",
    "--win nsis --x64 --publish never",
  ]) mustInclude(fragment);
  const envGate = stepOffset("Validate exact nonsecret signing environment before release config import");
  const forbiddenGate = stepOffset("Confirm forbidden credential environment is absent");
  const versionGate = stepOffset("Verify exact package version before production staging");
  const release = stepOffset("Build signed NSIS x64 candidate with electron-builder 26.15.7");
  const login = stepOffset("Establish Azure CLI session late after offline gates");
  const subject = stepOffset("Verify Azure certificate subject after login before signing");
  const native = stepOffset("Native pre-sign fixed Windows modules");
  assert.ok(envGate < forbiddenGate && forbiddenGate < versionGate
    && versionGate < login && login < subject && subject < native && native < release);
  const versionSection = stepSection("Verify exact package version before production staging");
  assert.match(versionSection, /RELEASE_VERSION/u);
  assert.match(versionSection, /TIBOTATTLE_PACKAGE_VERSION=/u);
  assert.match(workflow, /--profile windows-production/u);
  assert.match(workflow, /--version \$env:TIBOTATTLE_PACKAGE_VERSION/u);
  assert.ok(workflow.indexOf("electron-builder.release.config.cjs") > forbiddenGate);
  const envSection = stepSection("Validate exact nonsecret signing environment before release config import");
  for (const value of [
    "b7f8b18a-4338-40cb-a53a-6a05499be330",
    "485c1020-7234-4307-88ee-67294114f087",
    "8f6118f5-3c88-433d-a2c2-9f4b2aef8b23",
    "tibotattlesigning",
    "tibotattle-windows-public",
    "https://eus.codesigning.azure.net/",
    "Adam Allcock",
    "http://timestamp.acs.microsoft.com",
  ]) assert.ok(envSection.includes(value), `exact Azure value must be checked: ${value}`);
  const loginSection = stepSection("Establish Azure CLI session late after offline gates");
  assert.match(loginSection, /azure\/login@f5d393ae46f8fde4be8b75f32e3fc50e654ad0ca # v3\.0\.1/u);
  assert.doesNotMatch(loginSection, /# v2\.3\.0/u);
  assert.ok(loginSection.includes(`AZURE_CONFIG_DIR: ${ATTEMPT_AZURE_CONFIG_DIR}`));
  const buildSection = stepSection("Build signed NSIS x64 candidate with electron-builder 26.15.7");
  assert.ok(buildSection.includes(`AZURE_CONFIG_DIR: ${ATTEMPT_AZURE_CONFIG_DIR}`));
  const subjectSection = stepSection("Verify Azure certificate subject after login before signing");
  assert.ok(subjectSection.includes(`AZURE_CONFIG_DIR: ${ATTEMPT_AZURE_CONFIG_DIR}`));
  const nativeSection = stepSection("Native pre-sign fixed Windows modules");
  assert.ok(nativeSection.includes(`AZURE_CONFIG_DIR: ${ATTEMPT_AZURE_CONFIG_DIR}`));
  assert.equal(
    workflow.split("\n").filter((line) => line.includes(`AZURE_CONFIG_DIR: ${ATTEMPT_AZURE_CONFIG_DIR}`)).length,
    4,
  );
  assert.doesNotMatch(workflow, /AZURE_CONFIG_DIR=.*GITHUB_ENV/u);
  const sign = jobSection("sign");
  const loginOffset = stepOffset("Establish Azure CLI session late after offline gates", sign);
  assert.doesNotMatch(sign.slice(0, loginOffset), /AZURE_CONFIG_DIR=.*GITHUB_ENV/u);
  assert.match(loginSection, /audience: api:\/\/AzureADTokenExchange/u);
  assert.doesNotMatch(loginSection, /release-builder|electron-builder|Invoke-TrustedSigning/iu);
  assert.match(subjectSection, /AZURE_CODE_SIGNING_SUBJECT_SHA256: \$\{\{ vars\.AZURE_CODE_SIGNING_SUBJECT_SHA256 \}\}/u);
  assert.match(subjectSection, /-cnotmatch '\^\[0-9a-f\]\{64\}\$'/u);
  assert.match(subjectSection, /certificate-subject-preflight\.json/u);
  assert.match(subjectSection, /subjectSha256 -cne \$expectedHash/u);
  assert.match(subjectSection, /policy\.certificateSubjectSha256 -cne \$result\.subjectSha256/u);
  assert.ok(
    subjectSection.indexOf("azure/login") < 0,
    "certificate subject preflight must be a post-login step, not a login action",
  );
  const seedSection = stepSection("Seed canonical fixed evidence inputs without clobbering");
  assert.match(seedSection, /certificateSubjectSha256: process\.env\.TIBOTATTLE_VALIDATED_CERTIFICATE_SUBJECT_SHA256/u);
  const inputSection = stepSection("Build native pre-sign input offline");
  assert.match(inputSection, /--certificate-subject-sha256 \$env:TIBOTATTLE_VALIDATED_CERTIFICATE_SUBJECT_SHA256/u);
  assert.doesNotMatch(subjectSection, /subjectName|subjectDn|distinguishedName/iu);
  assert.match(workflow, /This is the only job with id-token: write/u);
  assert.doesNotMatch(workflow, /id-token: write is job-wide|Target code can mint an OIDC token/iu);
  assert.doesNotMatch(jobSection("prepare"), /id-token: write/u);
});

test("validated source revision is bound in every step that uses it", () => {
  const sourceRevisionUseSteps = [];
  const lines = workflow.split("\n");
  let currentStep = null;
  let currentSection = [];
  const finish = () => {
    if (currentStep !== null) {
      const section = currentSection.join("\n");
      if (section.includes("$env:SOURCE_REVISION")) {
        sourceRevisionUseSteps.push({ name: currentStep, section });
      }
    }
  };
  for (const line of lines) {
    const marker = line.match(/^      - name: (.+)$/u);
    if (marker) {
      finish();
      currentStep = marker[1];
      currentSection = [line];
    } else if (currentStep !== null) {
      currentSection.push(line);
    }
  }
  finish();
  assert.ok(sourceRevisionUseSteps.length > 0, "workflow must use SOURCE_REVISION in target steps");
  for (const { name, section } of sourceRevisionUseSteps) {
    assert.match(
      section,
      /SOURCE_REVISION:\s*\$\{\{\s*inputs\.source_revision\s*\}\}/u,
      `${name} must bind SOURCE_REVISION from the exact dispatch input`,
    );
  }
});

test("selected download policy and clean roots are gated before artifact downloads", () => {
  const sign = jobSection("sign");
  const gate = stepOffset("Verify exact selected download policy and absent roots", sign);
  const firstDownload = sign.indexOf("uses: actions/download-artifact@");
  assert.ok(firstDownload > gate, "download policy gate must precede actions/download-artifact");
  const gateSection = stepSection("Verify exact selected download policy and absent roots", sign);
  for (const fragment of [
    "tibotattle-windows-production-finalizer-receipts/warm",
    "tibotattle-windows-production-finalizer-receipts/clean",
    "selection.download.action -cne 'actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c'",
    "selection.download.artifactExpectation -cne 'direct-v7'",
    "selection.download.githubToken -cne '${{ github.token }}'",
    "selection.download.destinationMustBeAbsentBeforeDownload -ne $true",
    "selection.download.digestMismatch -cne 'error'",
    "selection.download.mergeMultiple -ne $false",
  ]) assert.ok(gateSection.includes(fragment), `download gate must enforce ${fragment}`);
  assert.match(gateSection, /Test-Path -LiteralPath \$warmRoot/u);
  assert.match(gateSection, /Test-Path -LiteralPath \$cleanRoot/u);
  assert.match(
    gateSection,
    /if \(\(Test-Path -LiteralPath \$warmRoot\) -or \(Test-Path -LiteralPath \$cleanRoot\)\)/u,
  );
});

test("fixed evidence producers run in order and publish no candidate", () => {
  const orderedSteps = [
    "Create fresh fixed production and attempt roots",
    "Select source evidence after checkout",
    "Install locked dependencies",
    "Build production Windows binding and manifest",
    "Stage fresh production Windows Electron app",
    "Seed canonical fixed evidence inputs without clobbering",
    "Build native pre-sign input offline",
    "Install and verify exact TrustedSigning module",
    "Establish Azure CLI session late after offline gates",
    "Verify Azure certificate subject after login before signing",
    "Native pre-sign fixed Windows modules",
    "Build authority input offline",
    "Run closed authority driver",
    "Build signed NSIS x64 candidate with electron-builder 26.15.7",
    "Verify fixed packaged artifact CLI",
    "Run native Authenticode, installer, and aggregate receipt CLI",
    "Verify aggregate receipt is unpublished and installed lifecycle is not_run",
  ];
  let previous = -1;
  for (const name of orderedSteps) {
    const current = stepOffset(name);
    assert.ok(current > previous, `${name} must be after the preceding stage`);
    previous = current;
  }
  for (const [key, leaf] of Object.entries(WINDOWS_PRODUCTION_SIGNED_FINALIZER_RECEIPTS)) {
    if (key === "nativePresign") continue;
    mustInclude(leaf, `fixed ${key} leaf`);
  }
  mustInclude("verify-windows-production-packaged-artifact.mjs");
  mustInclude("packaged-artifact-receipt.json");
  mustInclude("build-windows-production-finalizer-receipt.mjs");
  mustInclude("windows-production-finalizer-receipt.json");
  mustInclude("WINDOWS_SIGNED_FINALIZER_AGGREGATE_OUTPUT_PREEXISTS");
  mustInclude("WINDOWS_SIGNED_FINALIZER_SERIALIZED_AUTHENTICODE_UNEXPECTED");
  mustInclude("production.ready -ne $false");
  mustInclude("distribution -cne 'unpublished'");
  mustInclude("-cne 'not_run'");
  mustInclude("classes.node");
  mustInclude("classes.unexpected");
});

test("receipt steps do not redirect output and no diagnostic or release publication exists", () => {
  for (const name of [
    "Verify fixed packaged artifact CLI",
    "Run native Authenticode, installer, and aggregate receipt CLI",
  ]) {
    const section = stepSection(name);
    assert.doesNotMatch(section, /(?:^|\s)[12]?>>?\s*[^=]|\bOut-File\b|\bSet-Content\b/imu);
  }
  for (const token of [
    "actions/upload-artifact",
    "gh release",
    "softprops/action-gh-release",
    "update-feed",
    "update feed",
    "npm version",
    "pnpm version",
    "git tag",
  ]) mustNotInclude(token, `publication token ${token}`);
  mustNotInclude("GITHUB_STEP_SUMMARY", "raw diagnostic summary upload");
  mustNotInclude("selector", "selector enablement");
  mustInclude("--publish never");
});

test("native Authenticode and installer evidence are composed only inside the aggregate process", () => {
  mustNotInclude(
    "node ./scripts/verify-windows-production-authenticode-inventory.mjs",
    "standalone Authenticode CLI invocation",
  );
  mustNotInclude(
    "node ./scripts/verify-windows-production-installer.mjs",
    "standalone installer CLI invocation",
  );
  const aggregate = stepSection("Run native Authenticode, installer, and aggregate receipt CLI");
  assert.match(aggregate, /build-windows-production-finalizer-receipt\.mjs/u);
  assert.match(aggregate, /AGGREGATE_OUTPUT_PREEXISTS/u);
  assert.match(aggregate, /SERIALIZED_AUTHENTICODE_UNEXPECTED/u);
  assert.ok(
    aggregate.indexOf("AGGREGATE_OUTPUT_PREEXISTS")
      < aggregate.indexOf("build-windows-production-finalizer-receipt.mjs"),
    "pre-existing serialized leaves must be rejected before native collection",
  );
});

test("cleanup is unconditional, exact-root, and verifies a clean checkout", () => {
  const cleanup = stepSection("Remove exact attempt and signed candidate roots; verify clean checkout");
  assert.match(cleanup, /if: \$\{\{ always\(\) \}\}/u);
  for (const root of [
    ".release-build/electron-production/windows-x64",
    "native/windows-filesystem/build",
    "tibotattle-windows-production-signed-source",
    "tibotattle-windows-production-signed-source-evidence",
    "tibotattle-windows-production-finalizer-receipts",
    "azure-config",
  ]) assert.ok(cleanup.includes(root), `cleanup must remove ${root}`);
  assert.match(cleanup, /IsPathFullyQualified/u);
  assert.match(cleanup, /Resolve-Path/u);
  assert.match(cleanup, /ReparsePoint/u);
  assert.match(cleanup, /Get-ChildItem -LiteralPath \$target\.Path -Force -Recurse/u);
  assert.match(cleanup, /WINDOWS_SIGNED_FINALIZER_CLEANUP_REQUIRED/u);
  assert.match(cleanup, /\$unsafe -contains \$target\.Path/u);
  assert.match(cleanup, /native reparse\/race qualification remains/u);
  assert.match(cleanup, /Remove-Item -LiteralPath \$target\.Path -Recurse -Force/u);
  assert.match(cleanup, /WINDOWS_SIGNED_FINALIZER_CLEANUP_INCOMPLETE/u);
  assert.match(cleanup, /git diff --quiet/u);
  assert.match(cleanup, /git diff --cached --quiet/u);
  assert.match(cleanup, /git ls-files --others --exclude-standard/u);
});

test("prepare job has no OIDC or protected environment and emits bounded manifest outputs", () => {
  const prepare = jobSection("prepare");
  assert.match(prepare, /preparation-handoff-chunk-count: \$\{\{ steps\.prepare_handoff\.outputs\.manifest-chunk-count \}\}/u);
  for (let index = 0; index < WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_COUNT; index += 1) {
    const suffix = String(index).padStart(2, "0");
    assert.match(
      prepare,
      new RegExp(`preparation-handoff-chunk-${suffix}: \\$\\{\\{ steps\\.prepare_handoff\\.outputs\\.manifest-chunk-${suffix} \\}\\}`, "u"),
    );
  }
  assert.doesNotMatch(prepare, /id-token\s*:/u);
  assert.doesNotMatch(prepare, /environment:\s*windows-production-signing/u);
  assert.match(prepare, /github\.ref/u);
  assert.match(prepare, /github\.sha/u);
  assert.match(prepare, /--source-run-id \$env:SOURCE_RUN_ID/u);
  assert.match(prepare, /--source-run-attempt \$sourceRunAttempt/u);
  assert.match(prepare, /--source-revision \$env:SOURCE_REVISION/u);
  assert.match(prepare, /--source-ref \$env:SOURCE_REF/u);
  assert.match(prepare, /--workflow-run-id \$env:GITHUB_RUN_ID/u);
  assert.match(prepare, /--workflow-run-attempt \$env:GITHUB_RUN_ATTEMPT/u);
  assert.match(prepare, /--source-handoff \$env:TIBOTATTLE_PREPARE_QUALIFICATION_HANDOFF_PATH/u);
  assert.match(prepare, /--package-json \(Join-Path \$env:GITHUB_WORKSPACE 'package\.json'\)/u);
  assert.match(prepare, /--lockfile \(Join-Path \$env:GITHUB_WORKSPACE 'pnpm-lock\.yaml'\)/u);
  assert.match(prepare, /source-run-attempt=\$sourceRunAttempt/u);
  assert.match(prepare, /workflow-run-id=\$env:GITHUB_RUN_ID/u);
  assert.match(prepare, /workflow-run-attempt=\$env:GITHUB_RUN_ATTEMPT/u);
  assert.doesNotMatch(prepare, /--mode prepare/u);
  assert.match(prepare, /windows-preparation-manifest-transport\.ps1/u);
  assert.match(prepare, /-Mode Encode/u);
  assert.doesNotMatch(prepare, /FromBase64String|SHA256|selectedChunks|chunkMaximum/u);
  assert.doesNotMatch(prepare, /manifest-base64=\$base64/u);
  assert.match(prepare, /\*> \$null/u);
  assert.match(prepare, new RegExp(WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_SCRIPT.replaceAll(".", "\\."), "u"));
  assert.match(preparationTransport, /\$MaximumRawBytes = 262144/u);
  assert.match(preparationTransport, /\$MaximumBase64Bytes = 393216/u);
  assert.match(preparationTransport, /\$ChunkMaximumBytes = 24576/u);
  assert.match(preparationTransport, /\$MaximumChunks = 16/u);
  assert.match(preparationTransport, /manifest-chunk-count=/u);
  assert.match(preparationTransport, /manifest-encoded-bytes=/u);
  assert.match(preparationTransport, /manifest-sha256=/u);
  assert.equal(WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_MAXIMUM_BYTES, 262144);
  assert.equal(WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_MAXIMUM_BASE64_BYTES, 393216);
  assert.equal(WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_MAXIMUM_BYTES, 24576);
  assert.equal(WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_COUNT, 16);
});

test("sign job receives, revalidates, and matches bounded prepare chunks before Azure login", () => {
  const sign = jobSection("sign");
  assert.match(sign, /needs\.prepare\.outputs\.preparation-handoff-chunk-count/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-handoff-manifest-bytes/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-handoff-encoded-bytes/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-handoff-sha256/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-source-run-attempt/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-workflow-run-id/u);
  assert.match(sign, /needs\.prepare\.outputs\.preparation-workflow-run-attempt/u);
  for (let index = 0; index < WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_COUNT; index += 1) {
    const suffix = String(index).padStart(2, "0");
    assert.match(sign, new RegExp(`PREPARATION_HANDOFF_CHUNK_${suffix}: \\$\\{\\{ needs\\.prepare\\.outputs\\.preparation-handoff-chunk-${suffix} \\}\\}`, "u"));
  }
  const match = stepSection("Revalidate carried prepare manifest chunks and rebuilt tree before Azure login", sign);
  assert.doesNotMatch(match, /PREPARATION_HANDOFF_BASE64(?:_|:)/u);
  assert.match(match, /windows-preparation-manifest-transport\.ps1/u);
  assert.match(match, /-Mode Decode/u);
  assert.doesNotMatch(match, /FromBase64String|selectedChunks|chunkMaximum|CHUNK_TOTAL_MISMATCH/u);
  assert.match(match, /--verify/u);
  assert.match(match, /--expected-sha256/u);
  assert.match(match, /--manifest \$attemptHandoffPath/u);
  for (const flag of [
    "--source-run-attempt",
    "--workflow-run-id",
    "--workflow-run-attempt",
    "--package-json",
    "--lockfile",
  ]) assert.match(match, new RegExp(flag, "u"));
  assert.match(match, /sourceRunMetadata\.runAttempt/u);
  assert.match(match, /sourceRunMetadata\.headSha/u);
  assert.match(match, /sourceRunMetadata\.ref/u);
  assert.match(match, /sourceRunMetadata\.databaseId/u);
  assert.match(preparationTransport, /WINDOWS_PREPARATION_MANIFEST_TRANSPORT_DIGEST_MISMATCH/u);
  assert.ok(
    stepOffset("Revalidate carried prepare manifest chunks and rebuilt tree before Azure login", sign)
      < stepOffset("Establish Azure CLI session late after offline gates", sign),
  );
  assert.match(workflow, /TIBOTATTLE_PREPARATION_HANDOFF_SHA256=/u);
});

test("preparation chunk contract covers exact boundary, missing, and tamper cases", () => {
  const chunkMaximum = WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_MAXIMUM_BYTES;
  const maximumEncoded = "A".repeat(
    WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_MAXIMUM_BASE64_BYTES,
  );
  const chunks = Array.from(
    { length: WINDOWS_PRODUCTION_SIGNED_FINALIZER_PREPARATION_CHUNK_COUNT },
    (_, index) => maximumEncoded.slice(index * chunkMaximum, (index + 1) * chunkMaximum),
  );
  assert.equal(chunks.length, 16);
  assert.equal(chunks.every((chunk) => chunk.length === chunkMaximum), true);
  assert.equal(chunks.join(""), maximumEncoded);
  assert.equal(
    createHash("sha256").update(chunks.join(""), "utf8").digest("hex"),
    createHash("sha256").update(maximumEncoded, "utf8").digest("hex"),
  );
  const missing = [...chunks];
  missing[15] = "";
  assert.notEqual(missing.join("").length, maximumEncoded.length);
  const tampered = [...chunks];
  tampered[0] = `B${tampered[0].slice(1)}`;
  assert.notEqual(
    createHash("sha256").update(tampered.join(""), "utf8").digest("hex"),
    createHash("sha256").update(maximumEncoded, "utf8").digest("hex"),
  );
  const sign = stepSection(
    "Revalidate carried prepare manifest chunks and rebuilt tree before Azure login",
  );
  for (const marker of [
    "WINDOWS_PREPARATION_MANIFEST_TRANSPORT_CHUNK_INVALID",
    "WINDOWS_PREPARATION_MANIFEST_TRANSPORT_UNEXPECTED_CHUNK",
    "WINDOWS_PREPARATION_MANIFEST_TRANSPORT_CHUNK_TOTAL_MISMATCH",
    "WINDOWS_PREPARATION_MANIFEST_TRANSPORT_DIGEST_MISMATCH",
    "[Convert]::FromBase64String",
  ]) assert.match(preparationTransport, new RegExp(marker.replaceAll(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(sign, /-Mode Decode/u);
});

test("split workflow does not transfer binaries, cache, diagnostics, or publication", () => {
  for (const token of [
    "actions/upload-artifact",
    "actions/cache",
    "gh release",
    "softprops/action-gh-release",
    "GITHUB_STEP_SUMMARY",
  ]) mustNotInclude(token, `forbidden split-workflow token ${token}`);
  assert.match(workflow, /no binary|noBinary|content-free/iu);
  assert.match(workflow, /--publish never/u);
});
