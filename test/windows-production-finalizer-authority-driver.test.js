...
import {
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_BINDING_PATH,
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_KEYTAR_PATH,
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_LOCKFILE_PATH,
  WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PACKAGE_PATH,
...
const CERTIFICATE_SUBJECT_SHA256 = "f".repeat(64);
const PREPARATION_LOCKFILE_BYTES = 37;
...
function authenticode(overrides = {}) {
  return {
    ...
    subjectSha256: CERTIFICATE_SUBJECT_SHA256,
    subjectUtf8Bytes: 42,
...
function preparationHandoffBytes() {
  const binding = {
    bytes: BINDING_BYTES,
    path: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_BINDING_PATH,
    sha256: BINDING_SHA256,
  };
  const keytar = {
    bytes: 200,
    path: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_KEYTAR_PATH,
    sha256: WINDOWS_NATIVE_PRESIGN_KEYTAR_SHA256,
  };
  const prepared = buildWindowsProductionFinalizerPreparationHandoff({
    lockfile: {
      bytes: PREPARATION_LOCKFILE_BYTES,
      path: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_LOCKFILE_PATH,
      sha256: "8".repeat(64),
    },
    native: { filesystemBinding: binding, keytar },
    package: {
      bytes: 41,
      name: "app-usagemonitor",
      path: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PACKAGE_PATH,
      sha256: "9".repeat(64),
    ...
    qualification: {
      binding,
      receipts: {
        clean: { bytes: 444, sha256: CLEAN_RECEIPT_SHA256 },
        warm: { bytes: 333, sha256: WARM_RECEIPT_SHA256 },
      },
      revision: REVISION,
      run: SOURCE_RUN,
      runAttempt: SOURCE_RUN_ATTEMPT,
      status: "passed",
      workflow: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_WORKFLOW,
    },
    source: { ref: "refs/heads/main", revision: REVISION },
    staged: {
      files: [
        { bytes: 17, path: "app/main.js", sha256: "a".repeat(64) },
        binding,
        keytar,
      ].sort((left, right) => left.path.localeCompare(right.path)),
    },
    workflow: {
      path: WINDOWS_PRODUCTION_FINALIZER_PREPARATION_HANDOFF_PREPARATION_WORKFLOW,
      ref: "refs/heads/main",
      revision: REVISION,
      run: FINALIZER_RUN,
      runAttempt: 1,
  ...
  return Buffer.from(serializeWindowsProductionFinalizerPreparationHandoff(prepared), "utf8");
}

function preparationFacts() {
  return {
    handoff: { sha256: sha256(preparationHandoffBytes()) },
    source: {
      ref: "refs/heads/main",
      revision: REVISION,
      run: SOURCE_RUN,
    ...
    workflow: {
      path: ".github/workflows/windows-production-finalizer-signed.yml",
      ref: "refs/heads/main",
      revision: REVISION,
      run: FINALIZER_RUN,
      runAttempt: 1,
...
function fixture() {
  ...
  const selectedPreparationHandoffBytes = preparationHandoffBytes();
  const source = sourceRunMetadata();
  const facts = factsFor(selectedPresign);
  return {
    files: {
      handoff: selectedHandoffBytes,
      preparationHandoff: selectedPreparationHandoffBytes,
...
async function makeWorkspace() {
  ...
  await writeFile(join(root, "preparation-handoff.json"), value.files.preparationHandoff, { mode: 0o600 });
...
function options(root, facts) {
  return {
    ...
    preparationHandoff: "preparation-handoff.json",
...
test("supports the options document and fixed evidence-file flags", async (t) => {
  ...
  await runWindowsProductionFinalizerAuthorityArguments([
    ...
    "--output", "authority.json",
    "--handoff", "handoff.json",
    "--preparation-handoff", "preparation-handoff.json",
