...
const SUBJECT_SHA256 = sha256(Buffer.from(SUBJECT, "utf8"));
...
async function fixture() {
  ...
  const options = {
    ...
    certificateSubjectSha256: SUBJECT_SHA256,
    azure: {
...
test("builds a frozen, fixed-path, content-free pre-sign receipt", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      {
        ...
        certificateSubjectSha256: receipt.certificateSubjectSha256,
    ...
    const duplicate = serialized.replace(
      /("status":"WINDOWS_NATIVE_PRESIGN_PASSED")/u,
      '$1,"status":"WINDOWS_NATIVE_PRESIGN_PASSED"',
    );
    assert.throws(
      () => parseWindowsNativePresignReceipt(duplicate, binding),
      expectCode(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.inputInvalid),
    );
  } finally {
    await value.cleanup();
...
test("validates the closed receipt contract and every expected binding", async () => {
  ...
  try {
    ...
    for (const [field, replacement] of [
      ...
      ["certificateSubjectSha256", "f".repeat(64)],
    ]) {
      assert.throws(
...
test("constructs the exact pinned Trusted Signing command without a credential", async () => {
  ...
  try {
    ...
    assert.match(probe, /SignerCertificate\.Subject/u);
    assert.match(probe, /subjectSha256/u);
    assert.match(probe, /subjectUtf8Bytes/u);
    for (const azure of [
...
test("Authenticode aggregate is exact, publisher-bound, subject-bound, timestamped, and PA-verified", () => {
  const selected = validateAuthenticodeAggregate(authenticode(), PUBLISHER, SUBJECT_SHA256);
  assert.equal(selected.signerThumbprint, THUMBPRINT);
  assert.equal(selected.subjectSha256, SUBJECT_SHA256);
  ...
  for (const candidate of [
    ...
    assert.throws(
      () => validateAuthenticodeAggregate(candidate, PUBLISHER, SUBJECT_SHA256),
      expectCode(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid),
    );
  }
  assert.throws(
    () => validateAuthenticodeAggregate(authenticode({ subjectSha256: "0".repeat(64) }), PUBLISHER, SUBJECT_SHA256),
    expectCode(WINDOWS_NATIVE_PRESIGN_FIXED_STATUS.authenticodeInvalid),
  );
});
