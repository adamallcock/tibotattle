...
function preparationHandoff() {
  ...
  return buildWindowsProductionFinalizerPreparationHandoff({
    ...
    qualification: {
      binding,
      receipts: {
        clean: { bytes: 102, sha256: "c".repeat(64) },
        warm: { bytes: 101, sha256: "b".repeat(64) },
      },
      revision: REVISION,
      run: SOURCE_RUN,
      runAttempt: SOURCE_ATTEMPT,
...
async function fixture() {
  ...
  await Promise.all([
    ...
    writeFile(join(evidenceRoot, "preparation-handoff.json"), preparationBytes, { mode: 0o600 }),
...
test("derives driver facts and matches the direct authority builder", async (t) => {
  ...
  const preparationHandoffBytes = await readFile(join(value.evidenceRoot, "preparation-handoff.json"));
