...
const SUBJECT_SHA256 = "b".repeat(64);
const BINDING = {
...
test("verifies the exact packaged PE inventory and defers the standalone uninstaller", async () => {
  ...
  try {
    ...
    assert.equal(
      receipt.files.every(({ authenticode: rowAuth }) => rowAuth.subjectSha256 === SUBJECT_SHA256),
      true,
    );
    ...
    const duplicateSubjectKey = serialized.replace(
      `"subjectSha256":"${SUBJECT_SHA256}"`,
      `"subjectSha256":"${SUBJECT_SHA256}","subjectSha256":"${SUBJECT_SHA256}"`,
    );
    assert.throws(
      () => parseWindowsProductionAuthenticodeInventoryReceipt(duplicateSubjectKey, BINDING),
...
test("native PowerShell probe is closed and platform-gated", () => {
  ...
  assert.match(command, /SignerCertificate\.Subject/u);
  assert.match(command, /Encoding\]::UTF8\.GetBytes\(\$subject\)/u);
  assert.match(command, /subjectSha256 = \$subjectSha256/u);
  assert.match(command, /subjectUtf8Bytes = \[int64\]\$subjectBytes\.Length/u);
  assert.match(command, /signtool\.exe verify \/pa \/all/u);
  assert.doesNotMatch(command, /SignerCertificate \|/u);
  assert.doesNotMatch(command, /subject = \$signature\.SignerCertificate/u);
...
test("receipt rows recompute the aggregate and reject tampering", async () => {
  ...
  try {
    ...
    const subjectMissing = {
      ...receipt,
      files: receipt.files.map((row, index) => {
        if (index !== 0) return row;
        const { subjectSha256, ...withoutSubject } = row.authenticode;
        return { ...row, authenticode: withoutSubject };
      }),
    };
    assert.throws(
      () => validateWindowsProductionAuthenticodeInventoryReceipt(subjectMissing, BINDING),
      expectCode(STATUS.receiptInvalid),
    );
    const subjectOpen = {
      ...receipt,
      files: receipt.files.map((row, index) => index === 0
        ? { ...row, authenticode: { ...row.authenticode, subjectUtf8Bytes: 42, unexpected: true } }
        : row),
    };
    assert.throws(
      () => validateWindowsProductionAuthenticodeInventoryReceipt(subjectOpen, BINDING),
      expectCode(STATUS.receiptInvalid),
    );
    const signatureTampered = {
      ...receipt,
      files: receipt.files.map((row, index) => index === 0
        ? { ...row, authenticode: { ...row.authenticode, timestampPresent: false } }
        : row),
    };
    assert.throws(
      () => validateWindowsProductionAuthenticodeInventoryReceipt(signatureTampered, BINDING),
      expectCode(STATUS.receiptInvalid),
    );
