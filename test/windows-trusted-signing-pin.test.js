...
function validProbe(overrides = {}) {
  return {
    ...
    versions: ["0.5.0"],
    commandCount: 1,
    commandNames: ["Invoke-TrustedSigning"],
    commandModules: ["TrustedSigning"],
    commandVersions: ["0.5.0"],
...
test("TrustedSigning preflight is read-only, exact-version, and content-free", async () => {
  ...
  assert.match(command, /ExportedCommands/u);
  assert.match(command, /Invoke-TrustedSigning/u);
  assert.match(source, /REQUIRED_VERSION = "0\.5\.0"/u);
  assert.doesNotMatch(command, /Get-Command|Install-Module|Import-Module/u);
  assert.doesNotMatch(
    source,
  ...
  assert.equal(validateTrustedSigningProbe(validProbe()), true);
  assert.equal(
    validateTrustedSigningProbe(validProbe({ count: 2, versions: ["0.5.0", "0.6.0"] })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe(validProbe({ versions: ["0.5.1"] })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe(validProbe({ commandNames: [], commandCount: 0 })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe(validProbe({ commandModules: ["OtherModule"] })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe(validProbe({ commandVersions: ["0.5.1"] })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe(validProbe({ commandTypes: ["Alias"] })),
    false,
  );
  assert.equal(
    validateTrustedSigningProbe({ ...validProbe(), path: "C:\\private" }),
    false,
  );
  assert.equal(
    parseTrustedSigningProbe(JSON.stringify(validProbe())).status,
    REQUIRED_STATUS,
  );
  assert.equal(
    parseTrustedSigningProbe(JSON.stringify(validProbe({ commandModules: ["OtherModule"] }))).status,
    INVALID_STATUS,
  );
...
test("Windows preflight accepts only the exact injected success aggregate", () => {
  ...
  const outcome = runTrustedSigningPreflight({
    ...
    spawn(command, args, options) {
      ...
      return { status: 0, stdout: JSON.stringify(validProbe()) };
    },
  });
  ...
  assert.match(calls[0].args.join(" "), /ExportedCommands/u);
  assert.doesNotMatch(calls[0].args.join(" "), /Get-Command|Install-Module|Import-Module/u);

  const invalid = runTrustedSigningPreflight({
    platform: "win32",
    spawn: () => ({
      status: 0,
      stdout: JSON.stringify(validProbe({ commandVersions: ["0.5.1"] })),
    }),
  });
  ...
  const unavailable = runTrustedSigningPreflight({
    ...
    spawn: () => ({
      status: 1,
      stdout: "C:\\private\\module-path secret diagnostic",
      stderr: "private diagnostic",
    }),
  });
  assert.deepEqual(unavailable, { status: UNAVAILABLE_STATUS, exitCode: 1 });
  assert.equal(formatTrustedSigningPreflight(unavailable), `${UNAVAILABLE_STATUS}\n`);
});
