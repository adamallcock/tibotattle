...
const PREPARATION_HANDOFF_SHA = "8".repeat(64);
...
const CERTIFICATE_SUBJECT_SHA = "1".repeat(64);
...
function fixture(overrides = {}) {
  const base = {
    ...
    preparation: {
      handoff: { sha256: PREPARATION_HANDOFF_SHA },
      source: {
        ref: "refs/heads/main",
        revision: REVISION,
        run: 123456789,
    ...
    nativePresign: {
      ...
      certificateSubjectSha256: CERTIFICATE_SUBJECT_SHA,
    },
    runtimeManifest: {
...
function merge(base, overrides) {
  ...
  for (const key of ["sourceQualification", "sourcePackage", "preparation", "finalizer", "runtimeManifest", "signerPolicy"]) {
    if (overrides[key]) result[key] = { ...base[key], ...overrides[key] };
  }
  if (overrides.preparation?.handoff) {
    result.preparation.handoff = { ...base.preparation.handoff, ...overrides.preparation.handoff };
  }
  if (overrides.preparation?.source) {
    result.preparation.source = { ...base.preparation.source, ...overrides.preparation.source };
  }
  if (overrides.preparation?.workflow) {
    result.preparation.workflow = { ...base.preparation.workflow, ...overrides.preparation.workflow };
  }
...
test("builds a frozen content-free Windows authority snapshot", () => {
  ...
  assert.equal(Object.isFrozen(manifest.preparation), true);
  assert.deepEqual(manifest.preparation.handoff, { sha256: PREPARATION_HANDOFF_SHA });
...
test("binds the native presign receipt hash and provenance identity exactly", () => {
  ...
  for (const [field, value, code] of [
    ...
    ["certificateSubjectSha256", "Z".repeat(64), "invalid"],
  ]) {
  ...
  const missing = fixture();
  delete missing.nativePresign.certificateSubjectSha256;
  invalid(missing, "invalid");
});

test("binds the preparation handoff digest and both workflow identities", () => {
  const manifest = createWindowsProductionAuthorityManifest(fixture());
  assert.deepEqual(manifest.preparation, fixture().preparation);
  for (const [path, value, code] of [
    ["handoff.sha256", "A".repeat(64), "invalid"],
    ["source.run", 987654321, "mismatch"],
    ["source.workflow", WINDOWS_PRODUCTION_AUTHORITY_FINALIZER_WORKFLOW, "mismatch"],
    ["workflow.path", WINDOWS_PRODUCTION_AUTHORITY_SOURCE_WORKFLOW, "mismatch"],
    ["workflow.run", 987654322, "mismatch"],
  ]) {
    const candidate = fixture();
    const [section, key] = path.split(".");
    candidate.preparation[section][key] = value;
    invalid(candidate, code);
  }
  const extra = fixture();
  extra.preparation.handoff.extra = true;
  invalid(extra);
});
